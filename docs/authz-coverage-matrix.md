...
- **Arithmetic completeness (re-derived 2026-08-23, v1.11 Phase 2 folder-structure read, CURRENT):** **294 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 294`. By policy class: `owner_only` 133, `scoped_list` 39, `local_owner_only` **38**, `picture_scoped` 37, `public` 13, `any_token` 12, `loopback_owner_only` 6, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+3 `local_owner_only`** — `POST`, `GET .../status` and `DELETE /api/v1/folder-structure/read` — and retargets nothing; the other +4 since 2026-08-05 are v1.11 Phase 1's library-lifecycle routes, already rowed below. The delta from the 249 line below is larger than either, because this aggregate paragraph is **not** machine-checked while the rows are, so it went stale across everything merged between 2026-08-05 and today. That asymmetry is deliberate rather than an oversight: `tests/test_architecture_guardrails.py::test_coverage_matrix_document_matches_the_registry` fails the build in both directions on a missing or orphaned **row**, so coverage cannot drift; only this summary can, and a stale summary has never been the mechanism behind a BOLA. Restated rather than reconciled.
