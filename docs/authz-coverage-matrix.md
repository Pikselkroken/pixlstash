...
| Method | Effective path | Policy | id_param / body_ids | Rationale (current enforcement) |
|---|---|---|---|---|
| GET | `/api/v1/model-folders` | owner_only |  | Registered model folders, with a per-folder copy count. Reads host paths but takes none, so it stays on the shelf's `owner_only` tier rather than the locality tier, alongside `GET /reference-folders` and `GET /import-folders`. **Note it does NOT mirror `GET /libraries`:** that route redacts `path` to `null` for a remote owner and names redaction as its compensating control, whereas this one serves the host path in full to a remote owner, as `GET /adapters` does via `locations[].folder_path`. That is inside the trust-the-owner scope and matches the two sibling folder routes, but it is disclosure, not redaction |
