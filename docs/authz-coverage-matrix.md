...
- **Arithmetic completeness (re-derived 2026-09-09, root caption-file sync, CURRENT):** **312 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 312`. By policy class: `owner_only` 137, `local_owner_only` **47**, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `local_owner_only`** — `GET` and `PATCH /api/v1/server-config/captions`, the library root's copy of the four sidecar-sync fields `PATCH /reference-folders/{folder_id}` already carries — and retargets nothing. `tests/test_authz_host_capability_16_3.py::test_host_capability_tier_split_is_47_local_7_loopback` is the live assertion.
...
| GET | `/api/v1/server-config/captions` | **local_owner_only** |  | §16.3: reads back whether PixlStash writes caption files beside the pictures in the library's own root and under what filename suffix, and is the control surface of the PATCH beside it — the `GET /server-config/layout` argument exactly. Returns no per-object data and no path. On `READ_BLOCKED_GET_PATHS` for the same rollback reason as its layout sibling |
| PATCH | `/api/v1/server-config/captions` | **local_owner_only** |  | §16.3: decides whether, and under what filename, PixlStash writes a file beside every picture in the library root from here on, and asks for the root scan that does the first export. It takes no host path — the root is the library's own — but the suffix it stores is appended to picture paths on every later write, so it is validated to a bare filename fragment at the boundary (`is_safe_sidecar_suffix`), exactly as `PATCH /reference-folders/{folder_id}` validates the same four fields; the root's copy cannot sit on a lower tier than the folder's |
