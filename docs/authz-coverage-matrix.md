...
- **Arithmetic completeness (re-derived 2026-08-23, the workflow library view, CURRENT):** **291 declared**, covering the routes mounted in the default configuration plus the **1 conditionally-mounted** route (`POST /api/v1/test-hooks/ws-event`). Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 291`. By policy class: `owner_only` **137**, `scoped_list` 39, `picture_scoped` 37, `local_owner_only` 31, `public` 13, `any_token` 12, `character_scoped` 6, `loopback_owner_only` 6, `project_scoped` 6, `set_scoped` 4. The delta this change adds is exactly **+4 `owner_only`**, all on the new `workflows.py`: GET `/workflows`, GET `/workflows/{topology_hash}/variants`, GET `/workflows/{topology_hash}/pictures` and GET `/workflows/recipes/{structural_hash}/graph`. No other policy class moved and no existing declaration was retargeted. **The lines below were stale by more than this change adds** — the previous CURRENT line claimed 249 as of 2026-08-05 and the registry had grown to 287 before this branch touched it, so treat every figure under this one as a dated record rather than a running total.
- **Arithmetic completeness (re-derived 2026-08-05, #721 projected face routes, superseded by the line above):** **249 declared**, covering the **248** routes mounted in the default configuration plus **1 conditionally-mounted** route (`POST /api/v1/test-hooks/ws-event`). Measured, not carried forward from prose: `len(ROUTE_POLICIES) == 249`, `len(api_endpoint_set(app)) == 248`, `live - declared == ∅`, `declared - live == {POST /api/v1/test-hooks/ws-event}`. By policy class: `owner_only` **113**, `scoped_list` 39, `picture_scoped` **36**, `any_token` 14, `public` 13, `local_owner_only` 13, `character_scoped` **6**, `project_scoped` 6, `loopback_owner_only` 5, `set_scoped` 4. #721 itself adds exactly **+1 `picture_scoped`** (GET `/pictures/{id}/faces`) and **+1 `character_scoped`** (GET `/characters/{id}/faces`). The third route in the delta over the 246 below is **not from #721**: `POST /api/v1/dedup/verdicts/batch` (`owner_only`, so 112 -> 113) was declared on 2026-08-04 by `cb656898` "Make bulk dedup verdicts atomic", which did not re-derive this count. The 246 line was correct when written on 2026-08-02 and went stale two days later. That is exactly the drift the row-level check below now prevents for rows, though not yet for these aggregates. Every declared route now also has a table row.
