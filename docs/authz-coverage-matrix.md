...
- **Arithmetic completeness (re-derived 2026-09-18, #1398 saved recipes, CURRENT):** **336 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 336`. By policy class: `owner_only` **157**, `local_owner_only` 49, `picture_scoped` 43, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+5 `owner_only`**, the whole of the new `recipes.py` (`GET`/`POST /api/v1/recipes`, `PUT /api/v1/recipes/order`, `PATCH`/`DELETE /api/v1/recipes/{recipe_id}`), and retargets nothing. A saved recipe is the owner's own prompt and the listing's credit counts kept pictures across a whole stack of workflows, which is the same whole-library disclosure the `workflows.py` reads are owner-only for.
