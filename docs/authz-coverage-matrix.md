...
| POST | `/api/v1/workflows/run/preflight` | owner_only |  | The dry run of the above, submitting nothing. Declared on the same tier rather than one below it on purpose: it carries exactly the same disclosure - the resolved source, the grouping and the missing model filenames - so a scoped token allowed here would get the whole leak with none of the generation |
