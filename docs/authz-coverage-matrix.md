...
- **Arithmetic completeness (re-derived 2026-09-14, #1309 model-ghost purge, CURRENT):** **322 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 322`. By policy class: `owner_only` **145**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `DELETE /api/v1/server-config/ghost-retention/model-ghosts`, and retargets nothing.
