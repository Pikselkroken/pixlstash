...
| DELETE | `/api/v1/server-config/ghost-retention/ghosts` | owner_only |  | Owner privacy erase; DELETE is blocked for READ tokens, so only an unscoped owner reaches it. Destroys every picture ghost in the hub and nothing on disk, so owner data hygiene rather than a §16.3 host capability |
