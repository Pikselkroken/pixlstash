...
- **Arithmetic completeness (re-derived 2026-09-14, #1305 workflow picture-input modes, CURRENT):** **323 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 323`. By policy class: `owner_only` **146**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `owner_only`**, `GET` and `PUT /api/v1/comfyui/workflows/{workflow_name}/inputs`, and retargets nothing.
