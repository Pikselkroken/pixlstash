...
- **Arithmetic completeness (re-derived 2026-09-18, #1395 workflow cards, CURRENT):** **334 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 334`. By policy class: `owner_only` **155**, `local_owner_only` 49, `picture_scoped` 43, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+3 `owner_only`** on `workflows.py` — `GET /api/v1/workflows/cards`, `GET /api/v1/workflows/cards/{workflow_key}` and `GET /api/v1/workflows/cards/{workflow_key}/pictures` — and retargets nothing. All three read the whole vault: the grid is one `GROUP BY workflow_structural_hash` and one `ROW_NUMBER()` window over every non-deleted picture, so it discloses both the library's size one card at a time and a thumbnail URL for every card's best three pictures, which is the same whole-library class the four routes above already take this tier for.
