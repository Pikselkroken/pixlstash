...
- **Arithmetic completeness (re-derived 2026-09-13, the workflow library view, CURRENT):** **316 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 316`. By policy class: `owner_only` **141**, `local_owner_only` 47, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+4 `owner_only`**, all on the new `workflows.py`: GET `/workflows`, GET `/workflows/{topology_hash}/variants`, GET `/workflows/{topology_hash}/pictures` and GET `/workflows/recipes/{structural_hash}/graph`, and retargets nothing. The branch first counted 291 on 2026-08-23 against an older base; that figure is not carried forward, this one is re-derived after rebasing onto `develop`.
