...
| GET | `/api/v1/comfyui/workflows/{workflow_name}/inputs` | owner_only |  | Workflow setup; names the Fixed pictures by id; owner only |
| PUT | `/api/v1/comfyui/workflows/{workflow_name}/inputs` | owner_only |  | Workflow setup; PUT blocked for READ tokens; owner only |
...
| GET | `/api/v1/comfyui/workflows/{workflow_name}/parameters` | owner_only |  | Workflow parameters; reaches the owner's ComfyUI; owner only |
| PUT | `/api/v1/comfyui/workflows/{workflow_name}/pins` | owner_only |  | Workflow pins; PUT blocked for READ tokens; owner only |
| POST | `/api/v1/comfyui/workflows/{workflow_name}/run` | owner_only |  | Run a workflow; reads Fixed pictures across the library; owner only |
...
| Method | Effective path | Policy | id_param / body_ids | Rationale (current enforcement) |
|---|---|---|---|---|
| GET | `/api/v1/workflows` | owner_only |  | The library list: every workflow topology the hub knows, with how many of the **active vault's** kept pictures each accounts for. `owner_only` is a decision, not the default landing somewhere. The counts come from `topology_activity`, which groups every non-deleted picture in the vault: a picture-, set- or project-scoped token holding the response would learn the size of the whole library one workflow at a time, which is the whole-library disclosure class §16 exists for, and the service's own docstring refuses to be exposed any other way. The hub half discloses model filenames, which on a real shelf name people. Not §16.3: it takes no host path, walks nothing and reads no file — the hub is a database this process already owns |
| GET | `/api/v1/workflows/{topology_hash}/variants` | owner_only |  | The row's expansion: the recipes filed under one topology, each with the same vault-wide count. Same tier and the same two reasons as the list — a per-recipe count is the same disclosure narrowed to one graph, and the asset names are the same model filenames. The `{topology_hash}` is a content address, not an object id, so there is no membership to check: it is refused unless it is a 64-character hex digest, and a digest this hub has never filed is a 404 rather than an empty 200 |
| GET | `/api/v1/workflows/{topology_hash}/pictures` | owner_only |  | Picture **ids** for the inspector's "Made with it" tiles, newest first, capped at 60 by the route rather than by the caller. The plainest reason for the tier in the block: it returns picture ids straight out of the vault with no scope filter at all, so a scoped token would be handed ids its scope does not contain. It is deliberately not `scoped_list` — a scoped answer would need a narrowing parameter and a policy to check it against, which is work for whoever needs it and not something to invent here — and the route is not the way the grid reaches these pictures |
...
| PUT | `/api/v1/workflows/{workflow_key}/inputs` | owner_only |  | How a card's picture inputs are filled, kept per library. A `fixed` input names a picture by **content** (`pixel_sha`), which is why the setup is library-scoped: a second library gets its own rather than a picture it does not hold. Owner-only rather than picture-scoped for `POST /comfyui/workflows/{workflow_name}/run`'s reason - a content-addressed picture is not a body id a scope check could resolve |
