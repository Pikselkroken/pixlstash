...
- **Arithmetic completeness (re-derived 2026-09-26, #1523 the hand-made set merge offer merged over develop, CURRENT):** **364 declared**. Counted by importing `ROUTE_POLICIES` on the merged tree: `len(ROUTE_POLICIES) == 364`. By policy class: `owner_only` **187**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. Two `owner_only` routes over the 362 line below: this change's `PUT /api/v1/models/workflow-sets/{set_id}/declines`, and `PUT /api/v1/workflows/{workflow_key}/model-fix`, which reached `develop` without a line here. Neither is a GET, so the READ-token belt needs no entry.
- **Arithmetic completeness (re-derived 2026-09-26, #1520 hand-made workflow sets rebased onto the ComfyUI-PixlStash node check, superseded by the line above):** **362 declared**. Counted by importing `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 362`. By policy class: `owner_only` **185**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+5 `owner_only`** over the 357 line below - `POST /api/v1/models/workflow-sets`, `PATCH` and `DELETE /api/v1/models/workflow-sets/{set_id}`, `POST /api/v1/models/workflow-sets/{set_id}/members` and `POST /api/v1/models/workflow-sets/{set_id}/members/remove` - and retargets nothing. None is a GET, so the READ-token belt (`READ_BLOCKED_GET_PATHS`) needs no entry; the gate's `OWNER_ONLY` declaration is the whole enforcement.
- **Arithmetic completeness (re-derived 2026-09-26, ComfyUI-PixlStash node check merged over Edit with ComfyUI restored, superseded by the line above):** **357 declared**. Counted by importing `ROUTE_POLICIES` on the merged tree: `len(ROUTE_POLICIES) == 357`. By policy class: `owner_only` **180**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `GET /api/v1/comfyui/pixlstash-node`, and retargets nothing. It makes the server ask the owner's configured ComfyUI for its `/extensions` list, which is why it is not `any_token`; it is on the READ-blocked GET belt for the rollback.
- **Arithmetic completeness (re-derived 2026-09-26, Edit with ComfyUI restored, superseded by the line above):** **356 declared**. Counted by importing `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 356`. By policy class: `owner_only` **179**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `POST /api/v1/comfyui/workflows/{workflow_name}/card`, and retargets nothing. It files a stored workflow (user or built-in) on its card through the import's own `_file_in_hub`, the authority `POST /api/v1/comfyui/workflows/import` already has, and takes no host path. A POST, so READ tokens are refused by method.
- **Arithmetic completeness (re-derived 2026-09-25, #1530 convert an editor workflow to API format, superseded by the line above):** **355 declared**. Counted by importing `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 355`. By policy class: `owner_only` **178**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `POST /api/v1/comfyui/workflows/convert`, and retargets nothing. It writes stored workflow files through the import's own store, the authority `POST /api/v1/comfyui/workflows/import` already has, plus the converted graph beside the file; it takes no host path. A POST, so READ tokens are refused by method.
- **Arithmetic completeness (re-derived 2026-09-24, clone a workflow with new models rebased onto Open in ComfyUI, superseded by the line above):** **354 declared**. Counted by importing `ROUTE_POLICIES` on this branch after rebasing onto `develop`: `len(ROUTE_POLICIES) == 354`. By policy class: `owner_only` **177**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `owner_only`** over the 352 line below — `GET /api/v1/workflows/{workflow_key}/model-swap` and `POST /api/v1/workflows/{workflow_key}/clone-with-models` — and retargets nothing. Both resolve the card's graph out of the whole library as Duplicate does, and the GET also reads the whole shelf and every recipe on the hub. The GET is templated, so it is covered by the `/api/v1/workflows/` entry in `READ_BLOCKED_GET_PREFIXES` rather than needing an exact path.
- **Arithmetic completeness (re-derived 2026-09-24, Open in ComfyUI rebased onto #1478, superseded by the line above):** **352 declared**. Counted by importing `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 352`. By policy class: `owner_only` **175**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`** — `GET /api/v1/workflows/{workflow_key}/graph` — and retargets nothing. It resolves a card's graph the way `POST /workflows/run` does and hands it back without the export's scrub (credential widgets aside), so it is owner-only for the export's reason with more at stake; the `/api/v1/workflows/` prefix in `auth.READ_BLOCKED_GET_PREFIXES` already holds it if the gate is rolled back.
- **Arithmetic completeness (re-derived 2026-09-23, #1478 merged over #1440, superseded by the line above):** **351 declared**. Counted by importing `ROUTE_POLICIES` after merging `develop` into the #1478 branch: `len(ROUTE_POLICIES) == 351`. By policy class: `owner_only` **174**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. The two lines below each counted 349 on their own branch; together they add **+4 `owner_only`** over 347 (`GET` and `PUT /api/v1/workflows/{workflow_key}/lora-chain`, `POST` and `GET /api/v1/comfyui/workflows/pull`) and retarget nothing.
- **Arithmetic completeness (re-derived 2026-09-23, #1440 pull ComfyUI's saved workflows, superseded by the line above):** **349 declared**. Counted by importing `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 349`. By policy class: `owner_only` **172**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `owner_only`** — `POST /api/v1/comfyui/workflows/pull` and `GET /api/v1/comfyui/workflows/pull` — and retargets nothing. The POST reaches the owner's configured ComfyUI and writes stored workflow files through the import's own store, the authority `POST /api/v1/comfyui/workflows/import` already has; it takes no host path (the ComfyUI URL is the owner's own setting), so it is not `local_owner_only`. The GET is the untemplated half of the READ-token belt as well: it is in `auth.READ_BLOCKED_GET_PATHS` beside the `/api/v1/comfyui/workflows/` prefix that already refuses it.
- **Arithmetic completeness (re-derived 2026-09-23, #1478 the LoRA chain editor, superseded by the line above):** **349 declared**. Counted by importing `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 349`. By policy class: `owner_only` **172**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `owner_only`** — `GET /api/v1/workflows/{workflow_key}/lora-chain` and `PUT /api/v1/workflows/{workflow_key}/lora-chain` — and retargets nothing. Both resolve the card's graph the way `POST /workflows/{workflow_key}/insert-lora-loader` does, out of the whole library, name the shelf LoRA each loader loads, and reach the owner's ComfyUI for `object_info`; the PUT writes a new file and files a new card. The GET is templated under `/api/v1/workflows/`, which `READ_BLOCKED_GET_PREFIXES` already holds.
- **Arithmetic completeness (re-derived 2026-09-20 after rebasing onto `develop`, #1408 looks a workflow's pictures already carry, superseded by the line above):** **347 declared**. Counted from `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 347`. By policy class: `owner_only` **170**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`** — `GET /api/v1/recipes/used` — and retargets nothing; the diff against `develop` on `registry.py` is purely additive. It reads the same picture rows `GET /api/v1/recipes` already groups for credit and returns them grouped rather than counted. It also takes an entry in `auth.READ_BLOCKED_GET_PATHS`: `/api/v1/recipes/` is in `READ_BLOCKED_GET_PREFIXES` and refuses it today, but the prefix set is the **templated** half of that belt, and an untemplated owner-class GET is only held under the documented `AUTHZ_GATE_ENFORCING = False` rollback if its own path is in the frozenset — the rule `tests/test_authz_host_capability_16_3.py::test_every_untemplated_owner_class_get_is_on_the_read_blocked_belt` derives rather than writes down.

- **Arithmetic completeness (re-derived 2026-09-20 after a third rebase onto `develop`, #1438 model-shelf workflow sets, superseded by the line above):** **346 declared**. Counted by importing `ROUTE_POLICIES` on the rebased tree: `len(ROUTE_POLICIES) == 346`. By policy class: `owner_only` **169**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `GET /api/v1/models/workflow-sets`, and retargets nothing. **Counted rather than derived from the line below, and that matters here:** #1410's bullet says 344, but importing the registry at its own merge commit gives **345** (`owner_only` 168), so the figure below is one short of what that tree actually declared. 346 is 345 + this change's one route. This is the third time a bullet on this file has drifted - the matrix test parses the row tables and never these paragraphs, so nothing goes red - and it is why every figure here should be produced by importing the module rather than by adding to the previous line.
- **Arithmetic completeness (re-derived 2026-09-20, #1410 B9 retire the old workflow routes, superseded by the line above):** **344 declared**. Counted from `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 344`. By policy class: `owner_only` **168**, `local_owner_only` 49, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds nothing and retargets nothing: it removes **−11** declarations, **−8 `owner_only`** — `GET`/`PUT /api/v1/comfyui/workflows/{workflow_name}/inputs`, `GET /api/v1/comfyui/workflows/{workflow_name}/parameters`, `PUT /api/v1/comfyui/workflows/{workflow_name}/pins`, `POST /api/v1/comfyui/workflows/{workflow_name}/run`, `GET /api/v1/workflows/{topology_hash}/variants`, `GET /api/v1/workflows/{topology_hash}/pictures` and the topology list that used to hold `GET /api/v1/workflows` — and **−3 `picture_scoped`** — `POST /api/v1/comfyui/run_i2i`, `POST /api/v1/comfyui/run_t2i` and `POST /api/v1/comfyui/run_recipe`. The three card reads keep their tier and their rows and simply lose the `/cards` segment: `GET /api/v1/workflows`, `GET /api/v1/workflows/{workflow_key}` and `GET /api/v1/workflows/{workflow_key}/pictures`. Retiring a route removes a surface rather than widening one, and every run the deleted routes served is now `POST /api/v1/workflows/run`, which is `owner_only` and was already declared.
- **Arithmetic completeness (re-derived 2026-09-19, #1400 export / duplicate / insert loader / delete, superseded by the line above):** **355 declared**. Counted from `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 355`. By policy class: `owner_only` **176**, `local_owner_only` 49, `picture_scoped` 43, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+5 `owner_only`** — `GET /api/v1/workflows/{workflow_key}/export`, `POST /api/v1/workflows/{workflow_key}/duplicate`, `POST /api/v1/workflows/{workflow_key}/insert-lora-loader`, `DELETE /api/v1/workflows/{workflow_key}` and `GET /api/v1/recipes/{recipe_id}/export` — and retargets nothing. All five resolve a card's graph the way `POST /workflows/run` does, out of the linked file, the whole library's best picture or a stored instance document, so none of them is about an object a scoped token could hold. The two exports return a document in full: the workflow one is scrubbed (prose widgets, seeds, look LoRAs by name and by digest, node titles, picture names, output paths, shelf row ids, and any model name the shelf cannot vouch for), which is a privacy measure for whoever the file is given to and never a reason to widen the tier, and the recipe one is the owner's prompt verbatim.
- **Arithmetic completeness (re-derived 2026-09-18, #1395 workflow cards merged with #1398 saved recipes, superseded by the line above):** **339 declared**. Counted from `ROUTE_POLICIES` on the merged tree, not added up from the two bullets below: `len(ROUTE_POLICIES) == 339`. By policy class: `owner_only` **160**, `local_owner_only` 49, `picture_scoped` 43, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. #1395 adds exactly **+3 `owner_only`** on `workflows.py` — `GET /api/v1/workflows/cards`, `GET /api/v1/workflows/cards/{workflow_key}` and `GET /api/v1/workflows/cards/{workflow_key}/pictures` — and retargets nothing. All three read the whole vault: the grid is one `GROUP BY workflow_structural_hash` and one `ROW_NUMBER()` window over every non-deleted picture, so it discloses both the library's size one card at a time and a thumbnail URL for every card's best three pictures; the detail adds the parameter values the owner's best pictures were made with, read out of stored instance documents (capped at the newest `DEFAULT_SAMPLE` runs, so the read is bounded by the code rather than by how much somebody has generated). The other +5 are #1398's, rowed in the line below. **Re-derived by hand at the merge**, because `test_coverage_matrix_document_matches_the_registry` parses the row tables and never these bullets: two branches each adding rows at this anchor leave two stale `CURRENT` figures that no test contradicts.
- **Arithmetic completeness (re-derived 2026-09-18, #1398 saved recipes, superseded by the line above):** **336 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 336`. By policy class: `owner_only` **157**, `local_owner_only` 49, `picture_scoped` 43, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+5 `owner_only`**, the whole of the new `recipes.py` (`GET`/`POST /api/v1/recipes`, `PUT /api/v1/recipes/order`, `PATCH`/`DELETE /api/v1/recipes/{recipe_id}`), and retargets nothing. A saved recipe is the owner's own prompt and the listing's credit counts kept pictures across a whole stack of workflows, which is the same whole-library disclosure the `workflows.py` reads are owner-only for.
- **Arithmetic completeness (re-derived 2026-09-17, #1376 LoRA loader insertion, superseded by the line above):** **331 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 331`. By policy class: `owner_only` **152**, `local_owner_only` 49, `picture_scoped` 43, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `GET /api/v1/comfyui/workflows/{workflow_name}/lora-insertion`, and retargets nothing. Like its `/parameters` sibling it makes the server fetch the owner's configured ComfyUI `object_info`, which is why it is not `any_token`; the overlay does not ask it under a read-only share link, so that surface offers no insertion.
- **Arithmetic completeness (re-derived 2026-09-16, #1197 text in pictures, superseded by the line above):** **330 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 330`. By policy class: `owner_only` 151, `local_owner_only` 49, `picture_scoped` **43**, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `picture_scoped`**, `GET /api/v1/pictures/{id}/text` and `POST /api/v1/pictures/{id}/text/read`, and retargets nothing. The text is read out of the picture's own pixels, so a token that reaches the picture already reaches what it says; the POST only re-reads that derived text.
- **Arithmetic completeness (re-derived 2026-09-15, #1314 model companions merged with #1307 run a workflow, superseded by the line above):** **328 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 328`. By policy class: `owner_only` **151**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. #1314 adds exactly **+1 `owner_only`**, `POST /api/v1/models/companions`; the other +1 is #1307's, rowed in the line below. Neither retargets anything.
- **Arithmetic completeness (re-derived 2026-09-15, #1307 run a workflow, superseded by the line above):** **327 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 327`. By policy class: `owner_only` **150**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `POST /api/v1/comfyui/workflows/{workflow_name}/run`, and retargets nothing. It is owner-only rather than picture-scoped because it reads pictures two ways: by id from the body (`picture_ids` for the selection, `pictures` for Picker inputs), which a body-id scope check could cover, and by content from the stored setup for a Fixed input, which it could not. A scoped declaration would check the first and leave the second open, so the route takes the stricter class for both.
- **Arithmetic completeness (re-derived 2026-09-15, #1314 model companions, superseded by the line above; both 327 figures were counted before the other branch merged):** **327 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 327`. By policy class: `owner_only` **150**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `POST /api/v1/models/companions`, and retargets nothing.
- **Arithmetic completeness (re-derived 2026-09-15, #1306 workflow parameters, superseded by the line above):** **326 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 326`. By policy class: `owner_only` **149**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `owner_only`**, `GET /api/v1/comfyui/workflows/{workflow_name}/parameters` and `PUT /api/v1/comfyui/workflows/{workflow_name}/pins`, and retargets nothing. The GET makes the server fetch the owner's configured ComfyUI `object_info`, which is why it is not `any_token`; `DELETE /api/v1/comfyui/workflows/{workflow_name}` now also forgets that file's pins.
- **Arithmetic completeness (re-derived 2026-09-15, #1305 workflow picture-input modes merged with #1309 model-ghost purge, superseded by the line above):** **324 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 324`. By policy class: `owner_only` **147**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. #1305 adds exactly **+2 `owner_only`**, `GET` and `PUT /api/v1/comfyui/workflows/{workflow_name}/inputs`; the other +1 is #1309's, rowed in the line below. Neither retargets anything. Two widenings are deliberate: `GET /api/v1/comfyui/workflows` (`any_token`) now also says per file whether it has a Selection input, which names no picture and no library data beyond the file list it already served; and `DELETE /api/v1/comfyui/workflows/{workflow_name}` drops that file's picture-input setup in every library, not only the active one, because the file is one per machine and no library's setup for it survives it.
- **Arithmetic completeness (re-derived 2026-09-14, #1309 model-ghost purge, superseded by the line above):** **322 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 322`. By policy class: `owner_only` **145**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `DELETE /api/v1/server-config/ghost-retention/model-ghosts`, and retargets nothing.
- **Arithmetic completeness (re-derived 2026-09-13, #1293 ghost erase, superseded by the line above):** **321 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 321`. By policy class: `owner_only` **144**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `DELETE /api/v1/server-config/ghost-retention/ghosts`, and retargets nothing.
- **Arithmetic completeness (re-derived 2026-09-13, B4 picture-ghost retention, superseded by the line above):** **320 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 320`. By policy class: `owner_only` **143**, `local_owner_only` **49**, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `owner_only`**, `GET` and `PATCH /api/v1/server-config/ghost-retention`, siblings of the scrapheap-retention pair, and retargets nothing. The other **+2 `local_owner_only`** since the line below are other people's and already rowed; that line was not re-derived when they landed.
- **Arithmetic completeness (re-derived 2026-09-13, the workflow library view, superseded by the line above):** **316 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 316`. By policy class: `owner_only` **141**, `local_owner_only` 47, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+4 `owner_only`**, all on the new `workflows.py`: GET `/workflows`, GET `/workflows/{topology_hash}/variants`, GET `/workflows/{topology_hash}/pictures` and GET `/workflows/recipes/{structural_hash}/graph`, and retargets nothing. The branch first counted 291 on 2026-08-23 against an older base; that figure is not carried forward, this one is re-derived after rebasing onto `develop`.
- **Arithmetic completeness (re-derived 2026-09-11, library-root caption sync, superseded by the line above):** **312 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 312`. By policy class: `owner_only` 137, `local_owner_only` **47**, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `local_owner_only`**, `GET` and `PATCH /api/v1/server-config/captions`, the root's copy of the four caption-sync fields `PATCH /reference-folders/{folder_id}` already carries on this tier, and retargets nothing.
- **Arithmetic completeness (re-derived 2026-09-07, PixlStash Views withdrawn, superseded by the line above):** **310 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 310`. By policy class: `owner_only` 137, `local_owner_only` **45**, `picture_scoped` **41**, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change removes exactly **-2 `local_owner_only`** — `GET` and `PATCH /api/v1/server-config/views`, the two routes of the withdrawn PixlStash Views feature — and retargets nothing. The **+2** `picture_scoped` since the line below is other people's and was already rowed, which is why the total is unchanged at 310 rather than 308: **that coincidence is exactly why this paragraph is re-derived rather than patched**, since a delta reasoned from the previous total would have read as "nothing moved". The two removed rows are deleted from the table below; `GET /api/v1/server-config/views` also leaves `READ_BLOCKED_GET_PATHS` in `pixlstash/auth.py`, and the belt's membership is derived from `ROUTE_POLICIES` by `tests/test_authz_host_capability_16_3.py::test_every_untemplated_owner_class_get_is_on_the_read_blocked_belt`, so the two stay in step.
- **Arithmetic completeness (re-derived 2026-08-31, v1.11 Phase 4c library migration, superseded by the line above):** **310 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 310`. By policy class: `owner_only` 137, `local_owner_only` **47**, `scoped_list` 39, `picture_scoped` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `local_owner_only`** — `GET` and `POST /api/v1/server-config/layout/migration` — and retargets nothing. The `+1` in the line below (`DELETE /api/v1/folder-structure/commit`) is other people's and was already rowed; it did, however, land without moving `tests/test_authz_host_capability_16_3.py`'s locality counter, which was therefore red on `develop` and is corrected in this change rather than quietly folded in.
...
## Policy distribution (291 total; the 249 block below is the 2026-08-05 record)
...
| POST | `/api/v1/libraries/{library_uuid}/discard` | **local_owner_only** |  | Gives a folder back: deletes the temporary database and the thumbnail cache of a first import the owner did not keep, drops the registration, and lands the session on the app's own scratch library (`library_access=SWITCH_WRITER`). The one route here that **deletes files inside a host folder**, which is why it sits on the strictest tier its siblings hold a half of each - `POST /libraries`' write authority inside a folder, and `POST /libraries/active`' authority over every client's session. It takes a registry uuid, never a caller-supplied path, and the registry refuses anything but a library still on its first import (409), so the only files reachable are ones PixlStash created itself in a folder the owner had already named. **No picture file is touched** |
| POST | `/api/v1/libraries/{library_uuid}/promote` | **local_owner_only** |  | Finishes a library whose first import has not run: renames its database from the temporary name onto `vault.db` (`library_access=SWITCH_WRITER`). On this tier for both of its siblings' reasons at once - it renames a file inside a host folder, which is `POST /libraries`' write-authority class, and it closes and reopens the active vault to do it, which resets every connected client's session exactly as `POST /libraries/active` does. It takes a registry uuid, never a caller-supplied path, and the only file it touches is one PixlStash created itself inside a folder the owner had already named. A library that is already finished is refused (409), so this cannot rename anything else. Same `by_uuid` resolution as PATCH and DELETE |
...
| GET | `/api/v1/server-config/layout` | **local_owner_only** |  | §16.3 (v1.11 Phase 4b): reads back how this library's own picture root is laid out, and is the control surface of the PATCH beside it — the tier that alone may decide where the owner's files get written is the tier that may see the decision. Same argument as `GET /model-moves`. Returns no per-object data: a layout string and the unfiled folder name, never a path or a picture |
| PATCH | `/api/v1/server-config/layout` | **local_owner_only** |  | §16.3 (v1.11 Phase 4b): decides the folder names PixlStash writes into the library root from here on, and therefore where the background move engine later renames the owner's files to. It writes no file and moves nothing itself — the release's rule is that every path already in the library is true the moment it is written, so choosing a layout reorganises nothing — but the authority it hands out is host-filesystem authority, and the tier that may grant it is the tier that holds it. It accepts no host path at all: the root is the library's own. Sibling of `POST /server-config/layout/migration`, which acts on what it sets. A layout that cannot be parsed is refused 400 rather than stored, so a malformed one can never behave as "no layout" by accident |
| GET | `/api/v1/server-config/captions` | **local_owner_only** |  | §16.3: reads back whether PixlStash writes caption files beside the pictures in the library's own root and under what filename suffix, and is the control surface of the PATCH beside it, the same argument as `GET /server-config/layout`. Returns no per-object data and no path: two toggles and two suffixes |
| PATCH | `/api/v1/server-config/captions` | **local_owner_only** |  | §16.3: decides whether, and under what filename, PixlStash writes a file beside every picture in the library root from here on, and asks for the root scan that does the first export. The suffix it stores is appended to picture paths on every later write, so it is validated to a bare filename fragment here (no separators, no `..`, no media extension), as `PATCH /reference-folders/{folder_id}`'s is; that route is `local_owner_only` for these same fields, so the root's copy cannot sit lower. One suffix for both kinds is refused 400 |
...
| GET | `/api/v1/server-config/ghost-retention` | owner_only |  | Owner server-config read; returns no per-object data. Sibling of GET /server-config/scrapheap-retention. Reports a PRIVACY position — which thumbnails and prompts of destroyed pictures this install keeps — so owner_only rather than any_token |
| PATCH | `/api/v1/server-config/ghost-retention` | owner_only |  | Owner server-config write; PATCH is blocked for READ tokens, so only an unscoped owner reaches it. Sibling of PATCH /server-config/scrapheap-retention. Sets which picture ghosts a purge may keep and performs NO destruction itself |
| DELETE | `/api/v1/server-config/ghost-retention/ghosts` | owner_only |  | Owner privacy erase; DELETE is blocked for READ tokens, so only an unscoped owner reaches it. Destroys the active library's picture ghosts and nothing on disk, so owner data hygiene rather than a §16.3 host capability |
| DELETE | `/api/v1/server-config/ghost-retention/model-ghosts` | owner_only |  | Owner privacy purge; DELETE is blocked for READ tokens, so only an unscoped owner reaches it. Sibling of the picture-ghost erase. Forgets the model filenames hub recipes keep for models not on the shelf and nothing on disk |
...
| GET | `/api/v1/folder-structure/commit/status` | local_owner_only |  | §16.3: carries the commit's result, the same host-path class as `GET .../read/status`; owner + loopback/LAN/Tailscale, or remote owner iff `allow_remote_host_ops=true` (§16.3.1) `library_access=HUB_ONLY`: the commit state it reports lives on the Server, not in any vault, and it must keep answering while the library is closed and reopened, which is what the end of a first import does. It keeps the library pin, because what it returns is library content. |
| DELETE | `/api/v1/folder-structure/commit` | local_owner_only |  | §16.3: stops the owner's in-flight commit (abort, or 'organise later') — authority over another principal's operation, on the same tier as the route that starts it, exactly as `DELETE .../read` is to `POST .../read`; owner + loopback/LAN/Tailscale, or remote owner iff `allow_remote_host_ops=true` (§16.3.1) `library_access=HUB_ONLY` alongside its status sibling: it writes one flag on server-held state and touches no vault, and refusing it mid-swap would leave the owner unable to stop work they can still see running. It keeps the library pin. |
...
| GET | `/api/v1/pictures/{id}/text` | picture_scoped | id=id | Words read out of the picture with their boxes (#1197) |
| POST | `/api/v1/pictures/{id}/text/read` | picture_scoped | id=id | Re-reads the picture's text; derived data only (#1197) |
...
| POST | `/api/v1/stacks/keep-cover-only` | owner_only |  | Soft-deletes stack members to the Scrapheap and writes the metadata union (tags, score, pending character) onto their covers, across pictures named only by a stack or picture id. With `keep_recipes` + `keep_every_ghost` it also writes the server-wide `workflow_ghost_retention` privacy setting (the same write as PATCH /server-config/ghost-retention, which is owner_only too, so no privilege is gained). Stacks are set-membership-atomic, so this also changes what collections effectively contain. Same reasoning as POST /dedup/verdicts/stack and the mixed-stack mutations; POST is also blocked for READ tokens |
...
| GET | `/api/v1/comfyui/pixlstash-node` | owner_only |  | Whether the owner's ComfyUI has the ComfyUI-PixlStash node; reaches the owner's ComfyUI; owner only |
...
| POST | `/api/v1/comfyui/workflows/convert` | owner_only |  | ComfyUI's API conversion of an editor-format workflow (#1530), sent by the ComfyUI-PixlStash node: stored beside the matching stored editor file (stored first, as an import would, when there is none), never over it. Owner only, for the import's reasons |
| POST | `/api/v1/comfyui/workflows/pull` | owner_only |  | Pull ComfyUI's saved workflows (#1440): lists and reads them over the configured ComfyUI's userdata API and stores each in the user folder through the import's own store. Writes nothing to ComfyUI. Owner only, for the import's reasons plus reaching the owner's ComfyUI |
| GET | `/api/v1/comfyui/workflows/pull` | owner_only |  | The last pull's summary: counts, the card keys it filed on, and the node classes the owner's ComfyUI lacks. Owner only: it describes the owner's ComfyUI install and workflow library |
| DELETE | `/api/v1/comfyui/workflows/{workflow_name}` | owner_only |  | Delete workflow; DELETE blocked for READ tokens; owner only |
| GET | `/api/v1/comfyui/workflows/{workflow_name}/lora-insertion` | owner_only |  | LoRA loader insertion; reaches the owner's ComfyUI; owner only |
| POST | `/api/v1/comfyui/workflows/{workflow_name}/card` | owner_only |  | Files a stored workflow, user or built-in, on its Workflows card as an import would and returns the card key; Edit with ComfyUI opens the Run popup on the built-in image edit workflow through it. Writes the hub only; owner only, for the import's reasons |
...
| POST | `/api/v1/models/companions` | owner_only |  | The question in front of the shelf's Delete (#1314): given hub `model.id`s about to be deleted, which VAEs and text encoders they ran with would be left with nothing that uses them, which a kept model still uses, and which cannot be told apart from a same-named file. Reads hub rows only (`model`, `model_file`, `workflow_recipe_asset`) and changes nothing; it takes no host path and returns model names the shelf reads already serve, so it sits on the shelf read tier rather than the delete's §16.3 one. The recipe evidence spans every library on the hub, the same machine-level scope the shelf's own filenames already have |
| GET | `/api/v1/models/workflow-sets` | owner_only |  | The set grid's evidence (#1438): every distinct set of shelf models a kept picture proves ran together, read off the same `workflow_recipe_asset` self-join as `POST /api/v1/models/companions` above, plus one `GROUP BY workflow_structural_hash` and one `ROW_NUMBER()` window over the active library's non-deleted pictures for the counts and cover thumbnails. Owner-only for the reason the workflows grid's reads are: it sizes the whole vault one card at a time and returns a thumbnail URL for every card's best three pictures, and there is no per-object scope to narrow it to — a set is a fact about the machine's models rather than about a picture a token was granted. Reads only, and stores no membership: overlapping sets are derived per request |
| POST | `/api/v1/models/workflow-sets` | owner_only |  | Create a set, empty or with members (by `model.id`, or by sha256 for an undo). Hand-made workflow sets (#1520): the owner's own groupings of shelf models, stored in the hub (`model_workflow_set`, `model_workflow_set_member`). Writes hub set rows only, never a file or a `model` row, and answers with the set's evidence from the same whole-library read as `GET /api/v1/models/workflow-sets`, so it sits on that tier. Addressed by a hub set id no share token is ever scoped to. |
| PATCH | `/api/v1/models/workflow-sets/{set_id}` | owner_only |  | Rename a set. Hand-made workflow sets (#1520): the owner's own groupings of shelf models, stored in the hub (`model_workflow_set`, `model_workflow_set_member`). Writes hub set rows only, never a file or a `model` row, and answers with the set's evidence from the same whole-library read as `GET /api/v1/models/workflow-sets`, so it sits on that tier. Addressed by a hub set id no share token is ever scoped to. |
| DELETE | `/api/v1/models/workflow-sets/{set_id}` | owner_only |  | Delete a set; returns its snapshot for an undo. Hand-made workflow sets (#1520): the owner's own groupings of shelf models, stored in the hub (`model_workflow_set`, `model_workflow_set_member`). Writes hub set rows only, never a file or a `model` row, and answers with the set's evidence from the same whole-library read as `GET /api/v1/models/workflow-sets`, so it sits on that tier. Addressed by a hub set id no share token is ever scoped to. |
| POST | `/api/v1/models/workflow-sets/{set_id}/members` | owner_only |  | Add members. Hand-made workflow sets (#1520): the owner's own groupings of shelf models, stored in the hub (`model_workflow_set`, `model_workflow_set_member`). Writes hub set rows only, never a file or a `model` row, and answers with the set's evidence from the same whole-library read as `GET /api/v1/models/workflow-sets`, so it sits on that tier. Addressed by a hub set id no share token is ever scoped to. |
| POST | `/api/v1/models/workflow-sets/{set_id}/members/remove` | owner_only |  | Remove members by sha256. Hand-made workflow sets (#1520): the owner's own groupings of shelf models, stored in the hub (`model_workflow_set`, `model_workflow_set_member`). Writes hub set rows only, never a file or a `model` row, and answers with the set's evidence from the same whole-library read as `GET /api/v1/models/workflow-sets`, so it sits on that tier. Addressed by a hub set id no share token is ever scoped to. |
| PUT | `/api/v1/models/workflow-sets/{set_id}/declines` | owner_only |  | Replace the models kept out of a set's merge offer (#1523, Keep separate / Offer again); returns the previous list for an undo. Writes hub `model_workflow_set_decline` rows only, never a member, a file or a `model` row, and answers with the set's evidence from the same whole-library read as `GET /api/v1/models/workflow-sets`, so it sits on that tier. Addressed by a hub set id no share token is ever scoped to. |
...
| POST | `/api/v1/model-files/merge` | **local_owner_only** |  | §16.3 the per-copy delete (#1439): keeps the copy the body names and removes every other `present` copy of that model — OS trash by default, `permanent=true` unlinks — so it is `POST /model-files/delete`'s unlink narrowed to the copies the caller did **not** name, and sits on that route's tier for the same reason: it destroys the owner's bytes. It takes no host path (the body is a `model.id` and a `(folder_id, relpath)` the scanner wrote), every path is contained by the same `_contained_path`, and the naming is inverted on purpose — naming the keeper is what makes it impossible for a request to empty a model, and a keeper the shelf does not hold as `present` is refused rather than acted on. Only the copies being **removed** need a folder whose contents are the owner's (`user`/`managed`, or the download folder's unclaimed files), so keeping the shared HuggingFace cache's copy and removing a user-folder one is legitimate; a model with an `unreachable` copy is refused whole, as on the delete. **No hub row is deleted**: the removed copies keep their `model_file` rows at `state = 'removed'`, which is what keeps a recipe's filename resolving to the model afterwards. It also reads the owner's configured ComfyUI for its advertised model list, the same outbound read the run pre-flight makes on the same credential, and `dry_run=true` plans it and removes nothing. Owner + loopback/LAN/Tailscale, or remote owner iff `allow_remote_host_ops=true` (§16.3.1) |

### workflows.py (added 2026-08-23 — the workflow library view, plan §F1/§F2)

| Method | Effective path | Policy | id_param / body_ids | Rationale (current enforcement) |
|---|---|---|---|---|
| GET | `/api/v1/workflows` | owner_only |  | The Workflows grid (v1.12 B3): every card, its effective stacks, and the counts of what was left out. It took this route in B9 (#1410), which retired the topology list that used to hold it. `variant_activity` groups every non-deleted picture in the vault, and the cover strip hands back a **thumbnail URL** for each card's best three, which is the picture-ids disclosure of the row below repeated once per card and pointed straight at the image. It also names the owner's model files, which on a real shelf name people. Hidden and one-off cards are counted, never listed, so the count is the only thing a reader learns about them. No membership to check: a card key is a content address, and nothing here mutates |
| GET | `/api/v1/workflows/{workflow_key}` | owner_only |  | One card opened. It computes the whole grid to rank this card (the Bayesian prior is the library's own mean rating), so it reads exactly what the grid reads, and it adds what the grid does not: the **parameter values** the owner's best pictures were made with, read out of stored instance documents (capped at the newest `DEFAULT_SAMPLE` runs, so the read is bounded by the code rather than by how much somebody has generated). Those are the settings a person chose, which is the reason this cannot fall below the tier its list sits on. A hidden card still opens here — hiding is a decision about the grid, not a deletion — and a key this hub has never filed is a 404 rather than an empty 200 |
| GET | `/api/v1/workflows/{workflow_key}/pictures` | owner_only |  | Picture **ids** for one card, newest first, capped at 60 by the route rather than by the caller. The plainest reason for the tier in the block: ids straight out of the vault with no scope filter, so a scoped token would be handed ids its scope does not contain. It is deliberately not `scoped_list` — a scoped answer would need a narrowing parameter and a policy to check it against, which is work for whoever needs it and not something to invent here |
| GET | `/api/v1/workflows/recipes/{structural_hash}/graph` | owner_only |  | One recipe's stored structural document. Prompt-free and parameter-free by construction (`hub/schema.py`) and its assets are opaque references, so what leaks on a wrong tier is the **shape** of a graph the owner built plus its node count — still the owner's, and on the same tier as the two routes that list it. Not runnable in ComfyUI, and the payload says so rather than leaving a caller to find out; a corrupt stored document is a logged 500, never a 404, so "this machine does not have it" keeps meaning only that |
| PATCH | `/api/v1/workflows/{workflow_key}` | owner_only |  | The card writes (v1.12 B4) start here, and for them `owner_only` is not even a judgement about disclosure: this is the owner naming their own workflow, and there is no picture, set, character or project the decision belongs to, so no narrower policy could describe it. The verb carries a second belt - the READ-token middleware refuses PATCH before the gate reads a declaration - and the response is `GET /workflows/{workflow_key}`'s payload, so it discloses exactly what that row is owner-only for. An unknown key is a 404 rather than an attribute row written against a card this hub does not hold |
| PUT | `/api/v1/workflows/{workflow_key}/slots` | owner_only |  | Which of a card's LoRA slots are part of the workflow. **The one write here that changes identity**: a mark reaches the card key, so flipping one re-keys every card of the topology, and the owner's name, notes, pins, overrides, cover and stack membership are carried across in the same transaction (`hub/workflow_card_writes.flip_slot_marks`). Owner-only for the reason its siblings are, plus the plainer one that a scoped token re-keying somebody's library is not a disclosure question at all. It reads the vault's per-variant picture counts to decide which card keeps the name on a merge, which is the whole-library count class again |
| PUT | `/api/v1/workflows/{workflow_key}/model-fix` | owner_only |  | Replace a model a card's workflow names (a missing checkpoint, VAE or text encoder) with a shelf model, or undo that. Like the slot marks it **changes identity**: the replacement is read as the original in the card key, so every card of the topology is re-keyed in one transaction (`hub/workflow_card_writes.set_model_fix`) and the pictures already made with the replacement join the card. Owner-only for the slot marks' reasons: it rewrites what the owner's runs load and reads the vault's per-variant picture counts. Answers with the opened card, the same disclosure as the detail read |
| PUT | `/api/v1/workflows/{workflow_key}/defaults` | owner_only |  | The parameter values a card starts from, replaced whole. Owner-only as its siblings are, and it answers with the opened card so the new values come back marked `edited` - the same payload, and the same disclosure, as the detail read. The values are the owner's own settings, which is what `GET /workflows/{workflow_key}` is on this tier for |
| PUT | `/api/v1/workflows/{workflow_key}/pins` | owner_only |  | Which parameters a card's form shows before "All N". Addressed by `(slot label, input name)` and never by node id. It answers with what it was given rather than reading the card, so it discloses nothing beyond the request; the tier is the owner's decision about their own library |
| PUT | `/api/v1/workflows/{workflow_key}/inputs` | owner_only |  | How a card's picture inputs are filled, kept per library. A `fixed` input names a picture by **content** (`pixel_sha`), which is why the setup is library-scoped: a second library gets its own rather than a picture it does not hold. Owner-only rather than picture-scoped for the reason the retired `POST /comfyui/workflows/{workflow_name}/run` was (#1410): a content-addressed picture is not a body id a scope check could resolve. A pin may be sent as a `picture_id` (#1457); it is resolved against the kept pictures and stored as that picture's content, never as the id |
| POST | `/api/v1/workflows/{workflow_key}/unstack` | owner_only |  | Take one card out of its stack. Writes a `workflow_unstacked` row, which is a decision that survives a `CORE_VERSION` bump regrouping everything around it, and dissolves the stack it left if one card is now alone. Returns no per-object data beyond the key it was given; the tier is the owner rearranging their own library |
| GET | `/api/v1/workflows/{workflow_key}/graph` | owner_only |  | This card's graph as it runs, for *Open in ComfyUI* on the Workflow tab: the ComfyUI-PixlStash node fetches it when ComfyUI is opened with `?pixlstash_workflow=<key>`. Resolved as Run… resolves it (linked file, best kept picture, stored instance, against ComfyUI's own model list), and **not scrubbed** beyond blanking credential widgets — it is for the owner's own ComfyUI and is meant to run, which is Duplicate's reasoning too; the key is blanked because the graph crosses the network into ComfyUI's page. That makes it the most revealing read in the module, so it is owner-only with no narrower policy possible |
| GET | `/api/v1/workflows/{workflow_key}/export` | owner_only |  | This card's graph as a ComfyUI file (v1.12 B8). Owner-only for `POST /workflows/run`'s reason and one more: the graph is resolved out of the whole library — the linked file, the best kept picture's embedded metadata, or a stored instance document — so it is never one object a scope could name. The **scrub** (`services/workflow_export.py`) is not what makes this tier safe; it is what makes the FILE safe for whoever it is given to. It blanks every widget `workflow_hash.carries_prose` calls prose — **by name, across the whole graph, never by asking which node the sampler reads**, since that question returns nothing for a two-sampler hires-fix graph and names widgets an SDXL encoder does not have — nulls seeds, empties every LoRA slot not marked structural by filename **and by digest**, strips `_meta` titles, blanks picture file names, resets output paths, drops `checkpoint_id`, and drops any model name the shelf cannot vouch for along with the folder of the ones it can (`hub/workflows.unvouched_model_values`) — which is what stops a forgotten model name travelling back out of a picture's embedded graph. A graph that will not reduce is refused (409) rather than exported unscrubbed |
| POST | `/api/v1/workflows/{workflow_key}/duplicate` | owner_only |  | Writes this card's graph into the owner's workflow folder under a free name, so it can be changed in ComfyUI without touching the original — and so a card the library only knows from its pictures gets a file at all. Unscrubbed on purpose: the copy stays on this machine and is meant to run. Owner-only as its siblings are, plus the plainer one that writing into the owner's workflow folder is not a disclosure question; POST is blocked for READ tokens before the gate is reached |
| POST | `/api/v1/workflows/{workflow_key}/insert-lora-loader` | owner_only |  | The #1376 splice, written to a file: a copy of this card's graph with a LoRA loader wired in after the model source, so a workflow that had nowhere to put a LoRA now has a slot to swap into. It chooses no LoRA — the loader lands at ComfyUI's own widget defaults. Reaches the owner's configured ComfyUI for `object_info` (which is why it is not `any_token`, exactly as `/comfyui/workflows/{workflow_name}/lora-insertion` is not), refuses 409 where the splice cannot be made honestly, and never changes the original file |
| GET | `/api/v1/workflows/{workflow_key}/lora-chain` | owner_only |  | The LoRA chain editor's read (#1478): this card's LoRA loaders in the order a run applies them, their strengths, and the shelf LoRA each one loads (digest, or a unique case-folded basename match). The graph is resolved out of the whole library like its siblings', and the shelf digests are the owner's model inventory, so no scope could describe it. Reaches the owner's configured ComfyUI for `object_info`; unreachable is still a 200 with `editable: false` and the loaders read untyped |
| PUT | `/api/v1/workflows/{workflow_key}/lora-chain` | owner_only |  | The chain as the owner left it, written to a new file: loaders deleted (through `bypass_node`), added from the shelf, moved (node ids kept) and re-weighted, in one call and one new card. `dry_run` answers the change list and writes nothing. The original file is never changed. Refuses 409 where the chain cannot be edited honestly and 503 without ComfyUI; PUT is blocked for READ tokens before the gate is reached |
| GET | `/api/v1/workflows/{workflow_key}/model-swap` | owner_only |  | What the Clone with new models dialog draws: the model files this card's graph names, the shelf's checkpoints, VAEs and text encoders, and with `?checkpoint_id=` the VAEs and text encoders recipes on this machine, or ComfyUI runs read at the last workflow pull, have run beside it, plus the LoRAs and ControlNets trained on another family. The graph is resolved from the whole library as the export's is, and the proposal reads co-occurrence across every recipe on the hub, so none of it is about an object a scoped token could hold |
| POST | `/api/v1/workflows/{workflow_key}/clone-with-models` | owner_only |  | Duplicate with model filenames replaced: writes a copy of this card's graph into the owner's workflow folder, every loader naming a replaced file pointed at the new one, and names the new card as asked. Reaches the owner's configured ComfyUI for `object_info` to spell each name as ComfyUI lists it, and proceeds unchecked when it cannot; never changes the original file. POST is blocked for READ tokens before the gate is reached |
| DELETE | `/api/v1/workflows/{workflow_key}` | owner_only |  | Sends this card's imported workflow file to the system trash through the watched inbox and takes it off the card, sharing `routes/comfyui.py::trash_user_workflow` with `DELETE /comfyui/workflows/{workflow_name}` so there is one deletion path rather than two. Only the **user** folder resolves, so a built-in is a 404; a card with no file is a 409 ("hide it instead") rather than a silent success. The card, its variants and its pictures stay |
| POST | `/api/v1/workflows/run` | owner_only |  | Run a workflow card. Narrower than the `picture_scoped` run routes in `comfyui.py`: those replay one named picture's own embedded graph, so a scoped token is asking about its own object, while this resolves a graph from the WHOLE library - the linked workflow file, the library's best kept picture, or its stored instance documents - whatever the caller was scoped to. The body names LoRAs by shelf digest and the answer reports every card a selection grouped into together with the model filenames this ComfyUI is missing, which is the owner's model inventory. No per-object check could describe any of that. **It also reads and uploads pictures (#1457)**: by id from `picture_ids` and `inputs`, and by **content** from the card's stored `fixed` pins, each then uploaded into the owner's ComfyUI input folder. A pin is not a body id at all, so no body-id scope check could cover it, which is the same reason `PUT .../inputs` is owner-only |
| POST | `/api/v1/workflows/run/preflight` | owner_only |  | The dry run of the above, submitting nothing. Declared on the same tier rather than one below it on purpose: it carries exactly the same disclosure - the resolved source, the grouping, the missing model filenames and, since #1463, the node ids and class types of the LoRA loaders it took out (`bypassed_loras`) and of the seed nodes it replaced (`replaced_nodes`) - so a scoped token allowed here would get the whole leak with none of the generation, and since #1457 every picture input of the card with the picture each pin resolves to (`picture_inputs`) |
| POST | `/api/v1/workflows/stacks` | owner_only |  | Stack cards together. Each selected key expands to the stack it is already in, so stacking two stacks merges them, and the first key stays the cover. It reads the hub's whole card index to refuse an unknown key, and answers with the resulting member keys - card keys are content addresses, so that is the same class as the grid's own `member_keys` |
| PUT | `/api/v1/workflows/stacks/{stack_id}/order` | owner_only |  | Set a stack's member order by a complete ordered key list. Ordering an automatic grouping is what materialises it as a stack of its own, so the order survives a regrouping. `{stack_id}` is a minted id or `auto:<core hash>` and is refused unless it is one of those shapes; it is not an object id with a membership to check |
| POST | `/api/v1/workflows/stacks/{stack_id}/unstack` | owner_only |  | Dissolve a whole stack: every member stands on its own and stays out of the automatic grouping it came from. Answers with the member keys it took apart, which is the `member_keys` class the grid already serves on this tier. A stack this hub does not hold - including an `auto:` id whose group is empty - is a 404 rather than a silent success |

### recipes.py (added 2026-09-18 — saved recipes, plan §5.5, step B6)

| Method | Effective path | Policy | id_param / body_ids | Rationale (current enforcement) |
|---|---|---|---|---|
| GET | `/api/v1/recipes` | owner_only |  | The owner's saved recipes — their prompts, the LoRA filenames they load and the parameters they changed — plus **credit**, which counts the kept pictures of every workflow in the recipe's stack whose prompt and LoRA names match. That count is `workflows.py`'s whole-library disclosure in a narrower shape: a picture-, set- or project-scoped token holding it would learn how much of the library one look accounts for. `owner_only` is therefore a decision and not a default. Not §16.3: no host path, nothing walked, no file read |
| POST | `/api/v1/recipes` | owner_only |  | Saves a look against one `workflow_key`. Writes a vault row and nothing else; POST is blocked for READ tokens before the gate is reached, and the gate refuses every scoped token besides. The body's `source_picture_id` is checked to exist and then stored, never read back: an unknown id is a 422 rather than a foreign-key 500, a picture deleted later nulls the column rather than taking the recipe with it, and no picture data crosses the wire on this route |
| PUT | `/api/v1/recipes/order` | owner_only |  | Sets the order of the recipes the caller lists, by a complete ordered id list. Refused whole when an id is unknown, so a half-applied order is never left behind — and because it addresses recipes by id it is the one write here that could reach a row the caller did not create, which is precisely what `owner_only` closes |
| GET | `/api/v1/recipes/used` | owner_only |  | The looks the kept pictures of the named workflows' stacks were actually made with — each distinct prompt and LoRA-name combination, its picture count, one cover picture and whether a saved recipe keeps it (`saved`, a boolean about rows `GET /api/v1/recipes` already returns to the same caller) — minus the group with neither a prompt nor a LoRA, which matches everything and identifies nothing. **On both belts**: its own path in `READ_BLOCKED_GET_PATHS`, because an untemplated owner-class GET is not held by a prefix under the gate rollback, and under the `/api/v1/recipes/` prefix with its templated siblings. **The same picture rows as the credit figure on `GET /api/v1/recipes`, and a strictly WIDER disclosure of them.** That route groups these pictures and returns, per *saved* recipe, an integer; it says nothing at all about a look the owner never saved. This one returns those unsaved groups: the prompt verbatim, the LoRA file names, and a `cover_picture_id` that dereferences to a thumbnail. On the library this feature exists for — one that has never pressed Save — `GET /recipes` discloses nothing and this discloses every prompt in the stack. Same class, same tier, more of it: owner-only is therefore the same decision made a second time and not an inheritance, and nothing weaker was ever a candidate. A literal path declared **before** the `{recipe_id}` siblings in the router, as `/recipes/order` already is, so no templated route can shadow it. `workflow_key` repeats for a multiple selection and is capped at `MAX_UNION_KEYS`; stacks are folded before the read to keep the `IN` short, and an overlapping selection cannot count one look twice because both reads end in an `IN` over a set |
| PATCH | `/api/v1/recipes/{recipe_id}` | owner_only |  | Edits the fields the request carries. `workflow_key` is not writable, so a recipe cannot be moved onto another workflow, and `position` is not either — ordering is the route above. A recipe id is a vault row id with no scope of its own; the gate's owner check is the whole of its authorization |
| DELETE | `/api/v1/recipes/{recipe_id}` | owner_only |  | Forgets one recipe. The pictures it made are untouched — credit is computed on read from the picture rows, so there is nothing else to unlink |
| GET | `/api/v1/recipes/{recipe_id}/export` | owner_only |  | The recipe as a file, whole (v1.12 B8): prompt, negative, LoRA names with strengths, settings and seed. It withholds nothing — a recipe with those taken out makes nothing — so the response also carries `shares`, the plain list the export dialog shows before the owner agrees, including a line naming any model the shelf cannot vouch for, which is the same forgotten-name check the workflow export enforces by blanking. The row's id, its position in the tab and the picture it was saved from are dropped: they are this library's bookkeeping and mean nothing elsewhere. **It is also the first templated owner-class GET in `recipes.py`**, so this change adds `/api/v1/recipes/` to `auth.READ_BLOCKED_GET_PREFIXES` — the belt that keeps it closed if `AUTHZ_GATE_ENFORCING` is ever rolled back. Every other route in the module is a write, which the READ-token middleware refuses on the verb whatever the gate is doing; a GET returning the owner's prompt verbatim is not |
