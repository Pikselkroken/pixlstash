...
- **Arithmetic completeness (re-derived 2026-09-11, library-root caption sync, CURRENT):** **312 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 312`. By policy class: `owner_only` 137, `local_owner_only` **47**, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `local_owner_only`**, `GET` and `PATCH /api/v1/server-config/captions`, the root's copy of the four caption-sync fields `PATCH /reference-folders/{folder_id}` already carries on this tier, and retargets nothing.
...
| GET | `/api/v1/folder-structure/commit/status` | local_owner_only |  | §16.3: carries the commit's result, the same host-path class as `GET .../read/status`; owner + loopback/LAN/Tailscale, or remote owner iff `allow_remote_host_ops=true` (§16.3.1) **`library_independent=True`** for `GET /libraries`' reason: the commit state it reports lives on the Server, not in any vault, and it must keep answering while the library is closed and reopened - which is what the end of a first import does, so this is the only channel its own client has to be told what is happening. |
| DELETE | `/api/v1/folder-structure/commit` | local_owner_only |  | §16.3: stops the owner's in-flight commit (abort, or 'organise later') — authority over another principal's operation, on the same tier as the route that starts it, exactly as `DELETE .../read` is to `POST .../read`; owner + loopback/LAN/Tailscale, or remote owner iff `allow_remote_host_ops=true` (§16.3.1) **`library_independent=True`** alongside its status sibling: it writes one flag on server-held state and touches no vault, and refusing it mid-swap would leave the owner unable to stop work they can still see running. |
