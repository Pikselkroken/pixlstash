...
- **Arithmetic completeness (re-derived 2026-09-16, #1197 text in pictures, CURRENT):** **330 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 330`. By policy class: `owner_only` 151, `local_owner_only` 49, `picture_scoped` **43**, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `picture_scoped`**, `GET /api/v1/pictures/{id}/text` and `POST /api/v1/pictures/{id}/text/read`, and retargets nothing. The text is read out of the picture's own pixels, so a token that reaches the picture already reaches what it says; the POST only re-reads that derived text.
