...
- **Arithmetic completeness (re-derived 2026-08-24, v1.11 Phase 5 move reconciliation, CURRENT):** **306 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 306`. By policy class: `owner_only` **137**, `local_owner_only` 44, `scoped_list` 39, `picture_scoped` 39, `public` 13, `any_token` 12, `loopback_owner_only` 6, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+3 `owner_only`** — `GET /api/v1/moves/pending`, `POST /api/v1/moves/apply`, `POST /api/v1/moves/dismiss` — and retargets nothing.
