...
- **Arithmetic completeness (re-derived 2026-08-24, v1.11 Phase 3 folder-structure commit, CURRENT):** **298 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 298`. By policy class: `owner_only` 133, `scoped_list` 39, `local_owner_only` **42**, `picture_scoped` 37, `public` 13, `any_token` 12, `loopback_owner_only` 6, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `local_owner_only`** — `POST` and `GET .../status` on `/api/v1/folder-structure/commit` — and retargets nothing.
