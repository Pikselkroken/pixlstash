...
- **Arithmetic completeness (re-derived 2026-09-13, B4 picture-ghost retention, CURRENT):** **320 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 320`. By policy class: `owner_only` **143**, `local_owner_only` **49**, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+2 `owner_only`**, `GET` and `PATCH /api/v1/server-config/ghost-retention`, siblings of the scrapheap-retention pair, and retargets nothing. The other **+2 `local_owner_only`** since the line below are other people's and already rowed; that line was not re-derived when they landed.
...
| PATCH | `/api/v1/server-config/ghost-retention` | owner_only |  | Owner server-config write; PATCH is blocked for READ tokens, so only an unscoped owner reaches it. Sibling of PATCH /server-config/scrapheap-retention. Sets which picture ghosts a purge may keep and performs NO destruction itself (the purge in scrapheap_service is the only deleter) |
