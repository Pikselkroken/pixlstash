...
| GET | `/api/v1/workflows/cards` | owner_only |  | The Workflows grid (v1.12 B3): every card, its effective stacks, and the counts of what was left out. Strictly more disclosing than `GET /workflows` on the same tier — `variant_activity` groups every non-deleted picture in the vault, and the cover strip hands back the picture **ids** of each card's best three, which is the `{topology_hash}/pictures` disclosure repeated once per card. Hidden and one-off cards are counted, never listed, so the count is the only thing a reader learns about them. No membership to check: a card key is a content address, and nothing here mutates |
| GET | `/api/v1/workflows/cards/{workflow_key}` | owner_only |  | One card opened. It computes the whole grid to rank this card (the Bayesian prior is the library's own mean rating), so it reads exactly what the grid reads, and it adds what the grid does not: the **parameter values** the owner's best pictures were made with, read out of stored instance documents. Those are the settings a person chose, which is the reason this cannot fall below the tier its list sits on. A hidden card still opens here — hiding is a decision about the grid, not a deletion — and a key this hub has never filed is a 404 rather than an empty 200 |
