...
- **Arithmetic completeness (re-derived 2026-09-13, #1293 ghost erase, CURRENT):** **321 declared**. Counted from `ROUTE_POLICIES`: `len(ROUTE_POLICIES) == 321`. By policy class: `owner_only` **144**, `local_owner_only` 49, `picture_scoped` 41, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`**, `DELETE /api/v1/server-config/ghost-retention/ghosts`, and retargets nothing.
