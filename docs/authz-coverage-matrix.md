...
- **Arithmetic completeness (re-derived 2026-09-26, #1523 the hand-made set merge offer, CURRENT):** **363 declared**. Counted by importing `ROUTE_POLICIES` on this branch: `len(ROUTE_POLICIES) == 363`. By policy class: `owner_only` **186**, `local_owner_only` 50, `picture_scoped` 40, `scoped_list` 39, `public` 13, `any_token` 12, `loopback_owner_only` 7, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This change adds exactly **+1 `owner_only`** over the 362 line below, `PUT /api/v1/models/workflow-sets/{set_id}/declines`, and retargets nothing. Not a GET, so the READ-token belt needs no entry.
