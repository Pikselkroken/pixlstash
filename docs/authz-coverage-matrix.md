...
- **Arithmetic completeness (re-derived 2026-08-24, export-to-folder #291 merged onto the move engine, CURRENT):** **304 declared**. Counted from `ROUTE_POLICIES`, not carried forward from prose: `len(ROUTE_POLICIES) == 304`. By policy class: `owner_only` **134**, `local_owner_only` **44**, `scoped_list` 39, `picture_scoped` **39**, `public` 13, `any_token` 12, `loopback_owner_only` **7**, `character_scoped` 6, `project_scoped` 6, `set_scoped` 4. This branch itself adds exactly **+1 `loopback_owner_only`** — `POST /api/v1/pictures/export/folder`, which writes the exported pictures straight onto the host disk and then opens the destination in the host file manager, the same host-GUI spawn as the tier's other file-manager routes (`pictures/{id}/open-location`, `reference-folders/{folder_id}/open`, `models/{model_id}/open-location`, `server-config/open`) — on top of the 303/134/44/39/6 the line below already re-derived for the move engine; nothing else retargets.
