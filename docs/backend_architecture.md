...
| Body / form field | `POST /pictures/import`, `POST /pictures/import/staging`, `POST /reviews`, `POST /tag_suggestions/bulk-accept`, `POST /tag_suggestions/scan` | **nothing.** They are safe only because they are writes: a resource-scoped token can only be minted `READ` (§16.2 item 4) and `auth.py` blocks a non-`GET` for a `READ` token unless the path is in `READ_SAFE_POST_PATHS`, which none of these is. Adding one of them to that frozenset opens the hole. |
...
The body/form row is pinned by `tests/test_architecture_guardrails.py::test_project_references_outside_the_query_chokepoint`, which asserts the exact set in both directions *and* asserts none of them has become `READ`-reachable. It gates nothing; like `_KNOWN_WEBSOCKET_ROUTES`, it exists so a sixth such route cannot be added without someone looking at it.
