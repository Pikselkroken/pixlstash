...
| `workflow_origin` (written by `COMFYUI_WORKFLOW_PULL`, #1440; `hub/workflow_origin.py`) | Where a pulled workflow file came from: one row per `(origin, remote_path)`, the origin being the ComfyUI URL. `workflow_name` is **many-to-one** because the pull matches by content. `dismissed` is set by the delete path (`trash_user_workflow`) on **every** row naming the deleted file, and later pulls skip it; that is why it is its own table rather than columns on `workflow_file`, which the same delete drops. A path ComfyUI no longer lists is pruned and its local file kept. Only deletes made through PixlStash dismiss: a file removed from the folder by hand comes back on the next pull. `stored_by_pull` is 1 when the pull WROTE the file rather than matched one already stored; `card_index` reads it as `Card.hand_imported`, the one-off test's "imported" clause, so pulled workflows can be folded into the one-off count and a hand-imported file never is. `POST /comfyui/workflows/import` clears it for the name it stores or matches (`claim_file`): a file the owner hands over is theirs even if a pull wrote it first. Indexed on `workflow_name` (`ix_workflow_origin_name`), which every card read, delete and import looks up by. A path ComfyUI stops listing is pruned, so its file then reads as hand-imported |
