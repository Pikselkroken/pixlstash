...
| `workflow_topology_core` | Per topology: the automatic stack key (`core_hash` + `core_version`), the workflow type, and the slot list every mark and override addresses. **No filename and no asset reference** |
...
one automatic stack and `workflow_cards.card_grouping` counts them; the stack
tables hold the owner's own decisions. A `CORE_VERSION` bump therefore regroups
without destroying anything, and `STRIP_LORAS_FOR_STACKS` is the one line that
decides whether "the same workflow plus a character LoRA" stacks.
...
the filing: the picture's ingest and the file's import do not depend on it, and
the backfill retries.
