...
model type, or a branching chain. Only a plain one-slot loader wired into the
MODEL path is an editable link; any other node that loads a LoRA (a stacker, a
prompt tag, a character prompt builder) stays as an ordinary node the chain runs
around, so a loader can always be added between the model source and what reads
the model. A refused chain,
