...
- **Order.** Inputs are enumerated before anything rewires the graph (#1463's
  ...
  `pixlstash-{id}-{pixel_sha}{ext}` because ComfyUI's upload overwrites by name.
...
- **A selection feeding an input is the repeat axis.** The group runs once per
  ...
  (`stack_for_picture`) and the save node's `filename_prefix` is tagged with that
  stack and source, so an output a watched folder imports first still lands there.
