...
**A non-finite float never reaches the response.** `json.loads` accepts the `Infinity` and `NaN` literals and the response renders with `allow_nan=False`, so a crafted `prompt` chunk carrying `"cfg": NaN` would turn this read into a 500 for anyone holding a share token — the R3 threat model below, arriving through a field rather than a node class. `_finite_setting` and `_lora_strengths` refuse them, and the A1111 branch parses its text weights through the same guard.

**What `workflow_key` tells a share-token holder, stated rather than denied.** It is an opaque digest — no filename, no prompt, no count — over a graph that same token can already read whole from the `/workflow` sibling. It is **not** purely a function of that file: `workflow_key` folds in which LoRA slots the topology marks *structural*, and that mark was frozen from the filename of whichever picture of that topology was filed **first in this library** (`hub/schema.py`, `workflow_slot_mark`). A holder of two pictures differing only in a LoRA therefore learns one bit the files alone do not give them — whether that slot counts as part of the workflow — decided by a published rule over a filename. One bit per topology, no filename, nothing about a picture they cannot see. What a key *groups* — the "workflow: 8" comparison, the saved-recipe match banner — is an owner-only question answered by the card routes, not by this one.
