...
**Detection, not placeholders, classifies a workflow.** `GET /comfyui/workflows` takes `valid` and `workflow_type` from `detect_workflow_io` (`services/workflow_io.py`): a save node (`SAVE_NODE_CLASSES`, the classes output collection imports from) makes a workflow valid, a picture input (a class named `*LoadImage*`) makes it `i2i`. `missing_placeholders` is still reported as before, and the run routes still substitute placeholders until import stops rewriting workflows (#1303). Detection and placeholders can now disagree, so `run_i2i` refuses any workflow without `{{image_path}}`: a caption-only workflow with a fixed `LoadImage` lists as i2i, and running it would stack an output onto a picture the graph never read.
