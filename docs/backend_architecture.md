...
| [services/a1111_recipe.py](../pixlstash/services/a1111_recipe.py) | **A1111 generation data as a recipe** (#1312): parses a PNG's `parameters` text and classifies each field as structure, asset, parameter or volatile into a reduction `workflow_hash.graph_key` keys like a ComfyUI graph. Short model hashes are stored as written and resolved against the shelf when read |
...
ComfyUI graph in the file, `services/a1111_recipe.py` reads the PNG's
`parameters` text and builds the graph it describes as a reduction, which
...
prompts included. The known leak is a model named inside a compound value
(ControlNet's `Model: … [hash]`), which stays in the instance document where
forgetting the model does not reach it. **A short hash is kept
...
partial value in a ComfyUI widget still names nothing. Resolving at scan time would give two pictures of
one model different recipes depending on whether the model was on the shelf yet.
A1111's 8-digit hash from 2022 is not a sha256 prefix and is dropped. JPEG and
WebP are not read: A1111 puts their data in an EXIF sub-IFD
`extract_embedded_metadata` does not open. Migration `0119` hands back every PNG
stamped scanned with no keys, so existing A1111 pictures are read once.
