...
| [utils/device_utils.py](../pixlstash/utils/device_utils.py) | Inference device: `detect_device` (`cuda` or `cpu`), `is_accelerator` / `is_metal`, the allocator flush `empty_device_cache`, `configure_metal_model_loading` (§3), and the Apple Metal thread guard `register_metal_thread` / `ensure_metal_thread` (§7) |
