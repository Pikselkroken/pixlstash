...
- **A correction the hub's own CHECK would reject is refused by name.** `model` carries `CHECK (file_kind <> 'adapter' OR sha256 IS NOT NULL)` and the same for `kind`, so promoting an unhashed checkpoint to `adapter` violates a constraint. Left to SQLite that surfaces as a 500 naming `CHECK constraint failed`, which tells the owner nothing about the file they picked; the handler names the file instead.
