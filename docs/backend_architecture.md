...
│   ├── cpu_query_encoders.py         # CPU copies of SBERT/CLIP for search queries on Apple Metal
...
│   ├── device_utils.py                  # detect_device, Metal thread guard, cache flush
...
`utils/device_utils` is the single answer to "which device?". **`detect_device()` returns `cuda`, `mps` or `cpu` — never write `"cuda" if torch.cuda.is_available() else "cpu"` inline again.** That expression sees only the accelerators `torch.cuda` reaches (PyTorch's ROCm build drives AMD through the CUDA API), and Apple's Metal backend is not one of them, so the choice is made here, once, rather than at each call site. `is_accelerator(device)` is the "is this GPU-class?" test and accepts a string or a `torch.device`.

`utils/vram_utils.is_device_error(exc, device)` is the shared "should this retry on the CPU?" predicate. It is broader than `is_vram_oom()`, which answers only "the device ran out of memory": a build that cannot drive the installed card is not an OOM but has the same remedy. It answers `True` for any `is_vram_oom()` failure on an accelerator, and otherwise only for what a fault on **that** device says, walking `__cause__`/`__context__`:

- **`device` picks the phrase set** (`_DEVICE_FAULTS`), normalised like `is_accelerator` (`"mps:0"`, `torch.device`). `None` and `cpu` are never device errors, and every entry in `ACCELERATORS` must have a set.
- **CUDA:** `CUDA error` (cuBLAS statuses and "no kernel image" arrive inside it), `CUDA driver`, `CUDA unknown error`, `cuDNN error`, `cuDNN version` and `cuDNN Frontend error`, the ways cuDNN says no algorithm, engine or plan fits (`Unable to find a valid cuDNN algorithm`, `no valid convolution algorithms available`, a bare `CUDNN_STATUS_` code, and `was unable to find an engine`, which torch prefixes with the heuristic mode it asked for: `GET` by default, `FIND` when benchmarking), the caching allocator's `NVML_SUCCESS == r INTERNAL ASSERT FAILED` and `Triton Error [CUDA]`, plus `torch.cuda.CudaError` by type. Bare `cudnn`/`cublas` are not phrases: torch's argument checks name the cuDNN op a misplaced tensor reached, and cuBLAS configuration errors name the library.
- **Metal:** `Invalid buffer size` and the backend's other size limits (`not supported at the MPS device`, `MPSGraph does not support tensor dims larger than`), `not currently implemented for the MPS device` and `not implemented on MPS device yet`, and the dtype refusals (`not supported on MPS`, `MPS framework doesn't support float64`, `to the MPS backend but it does not have support for that dtype`, `scaled_dot_product_attention for MPS does not support`), beside the OOM `is_vram_oom()` reads. `Invalid buffer size` is one allocation larger than the device's maximum buffer; it is refused the same way on every attempt, so it is not an OOM and only the CPU retry helps. The rest are capability gaps, not failures; by decision they retry on the CPU, which can run what Metal cannot. `MPS device does not support linear for non-float inputs` and its kind are not matched: the CPU refuses an integer input to float weights too.
- **Bugs raise on every device.** A tensor on the wrong device (`found at least two devices, mps:0 and cpu!`, `expected on mps`), a mistyped input and a `view` that needed `reshape` name the device too, and a CPU retry would hide them while the GPU sat idle. A device's name is never matched.

Florence-2, CLIP, SBERT and the built-in tagger all route through it; a service that matches device-error strings itself is a bug. The table in `tests/test_mps_device.py::test_is_device_error` is the contract.

`utils/device_utils.configure_metal_model_loading()` sets `HF_DEACTIVATE_ASYNC_LOAD=1` whenever Metal exists, called first thing in `InferenceEngine.create`. transformers 5.x otherwise loads weights on a four-thread pool, and those threads casting on Metal at once crash or hang the process (`docs/apple-metal-thread-safety.md`). It is keyed on Metal being present, not on it being the engine's device, because accelerate's `device_map="auto"` places weights on Metal whenever it exists. A value already in the environment is kept, and one transformers reads as false (anything but `true`/`1`/`y`/`yes`/`on`, `0` and an empty string included) is logged as a warning.

`utils/device_utils.empty_device_cache()` is the shared cache-flush helper, covering CUDA and Metal, and deliberately reads `sys.modules.get("torch")` instead of importing: if torch was never imported, the process cannot hold device allocations, so there is nothing to free and importing torch to learn that would cost seconds on every teardown. It never raises: a backend whose probe or flush fails is logged at WARNING, naming the backend, and the call returns `False`.
...
| [pixlstash/startup_checks.py](../pixlstash/startup_checks.py) | Preflight: disk space, VRAM, GPU (CUDA / Metal), SSL. May force CPU mode. |
...
**Detection, not placeholders, classifies a workflow.** `GET /comfyui/workflows` takes `valid` and `workflow_type` from `detect_workflow_io` (`services/workflow_io.py`): a save node makes a workflow valid (the output collection's own rule: an explicit `pixlstash_output_nodes` choice, else `SAVE_NODE_CLASSES`), a picture input (a `*LoadImage*` class, case-insensitive, or a known loader such as `PixlStashPictureLoader`) makes it `i2i`. Each file is described once per `(path, mtime, size)`, so opening a menu does not re-reduce every graph.

**A workflow is stored as imported, and a run fills it without tokens** (#1303, `services/workflow_bindings.py`). `POST /comfyui/workflows/import` writes the JSON unchanged, UI or API format. A document not shaped like either is a 400 and is not written: `check_comfy_workflow` (`utils/comfyui_utilities.py`), called first in `_store_workflow`, wants a non-empty `nodes` list whose every node has an `id` and a `type`, beside a `links` list (or, in a schema-version-1 file that dropped an empty one, `state` or `last_node_id`), or (bare or under `prompt`) at least one node and every non-`pixlstash_*` entry a node with a `class_type` and an `inputs` object. A copy of a stored workflow (same content, PixlStash's own `pixlstash_*` keys ignored, built-ins included) comes back `matched` under the stored name instead of being written twice; a name taken by different content is a 409 unless `overwrite` or `keep_both` (`name (2).json`). The graph is filed in the hub (`record_api_graph`, or `record_ui_graph` for the topology of a UI file), which is what puts a dropped file on the Workflows view row its pictures already made; `topology_hash` in the response names that row. What a run fills comes from `run_targets`: a file's `pixlstash_bindings` when it has them, otherwise detection on an API-format graph (exactly one picture input; exactly one positive prompt's text, one link upstream when a primitive feeds it — two of either is ambiguous and fills nothing, so a fixed style prompt combined with the subject is never overwritten). An empty caption leaves the prompt as the workflow has it. `fill` writes each path once from every role bound to it, so a template holding both tokens gets both values. `missing_placeholders` keeps its name and now means *no target for that role*, so the ComfyUI menus still choose by what the run routes accept; `run_i2i` refuses a workflow with no picture target and `run_t2i` one with. **Bindings exist only for files the old import dialog wrote.** Start-up (`Server.DEFAULT_MIGRATE_WORKFLOW_TOKENS`, off in the suite because the folder is machine-global) runs `migrate_workflow_folder` once per folder. **The `.placeholder-bindings-migrated` marker is taken exclusively before the pass**, creating the folder on a fresh install, so an as-is import made after that start is never given empty bindings and a second process starting at once leaves the files alone; a file that could not be read or written for a reason that may pass (locked, disk full) is named in the marker's `retry` list and only those are tried at the next start, while a file that is not valid JSON is logged and left. Temp files are `mkstemp` names, removed on failure. In the pass, a file with tokens gets each token's JSON path and node as a binding and a neutral value back (`example.png`, an empty prompt); a file with none gets `pixlstash_bindings: []`, because the dialog let a workflow take no picture or no caption and detection must not start filling those. Each rewritten file keeps its original beside it as `<name>.json.pre-bindings`, which is also the way back for an older build, whose run routes need the tokens. A token inside a longer string is bound with that string as its `template`, so a run still substitutes into it; the stored value loses the token, the binding is `recovered: false` and the list reports the workflow `flagged`. `_meta` is never read for tokens. Import runs `migrate_placeholders` too, so an old export is stored the way its siblings were and matches them. Detection is a known reach until #1305: a caption-only workflow with a fixed reference `LoadImage` imported as-is now fills that loader, where one the old dialog stored keeps its bindings and stays t2i.

**The watched `workflows/` folder is an inbox** (#1304, `services/workflow_inbox.py`). It sits in the app's data directory beside `comfyui-workflows/`. A `.json` put there is stored through `_store_workflow`, the import route's own body, with `keep_both` as a drop has, then renamed `<stem>.<hash>.json`: `content_hash` is the first 16 hex of the sha256 of `workflow_bindings.canonical` over the migrated document, so it is the hash of the workflow as stored and a delete writes back the same name. The stem, less any hash, is the name it is stored under (or `stem (2).json` beside a different workflow; a copy matches the stored one whatever its name). A rename never replaces an existing file. Start-up (`Server.DEFAULT_WATCH_WORKFLOW_INBOX`, off in the suite because the folder is machine-global) runs `reconcile` after the placeholder migration and then starts `WorkflowInboxWatcher`, a watchdog observer that reconciles one second after the last `.json` event. `reconcile` is idempotent: an imported file matches its stored copy and already carries its name. It re-stores every inbox file on each pass (a match per file against every stored workflow), and editing a file in the inbox stores each saved version beside the last. The import route holds `INBOX_LOCK` too. A file that is not a JSON object, or not a workflow, is logged and left. **Removing a file never deletes a workflow.** `DELETE /comfyui/workflows/{name}` goes the other way: under `INBOX_LOCK`, which every reconcile also holds, it writes the stored workflow back to a name no file has yet (`<stem>.<hash>.json`, else `<stem> (2).<hash>.json`), sends every inbox file whose *contents* hash to that workflow to the system trash (never judged by name, so a file edited in place and not yet reconciled stays for the watcher) (`send2trash`), and only then removes the stored file, so a copy dropped under another name cannot import the workflow again and a trash that refuses is a 500 with the workflow kept. A stored file that is not a readable workflow is trashed as it is. Restoring a trashed file puts it back in the inbox, which imports it again. The Workflows list does not refresh on a watched import; it shows the workflow the next time it is fetched.

**How each picture input is filled (#1305).** Every detected picture input of a saved workflow file has a mode: `selection` (the grid's selection fills it; at most one per workflow), `picker` (asked at run time) or `fixed` (one picture chosen at setup). `GET` / `PUT /comfyui/workflows/{workflow_name}/inputs` (both `OWNER_ONLY`) read and replace the setup; the resolution and validation rules live in `services/workflow_inputs.py`. The modes are stored in the hub's `workflow_picture_input`, beside the file and never in it. Inputs nothing stored names default to Picker, except that with nothing stored (or a stored Selection naming a node the file has lost) the input a run already fills with the picture (its migrated binding, or the one detected picture input; `workflow_bindings.run_targets`), else the lowest node id, is Selection. `PUT` names every input exactly once; a `fixed` entry sends a `picture_id` to choose a picture, or none to keep the one already stored, even one that has since left the library, so changing another input never drops it. An input that leaves Fixed keeps its picture too, and `GET` returns it as that input's `picture_id`, so stepping off Fixed with one arrow key and back costs nothing. Rows are keyed on the file's own spelling of its name (`_on_disk_name`), not the client's, so a case-insensitive filesystem cannot split one file's setup. `GET /comfyui/workflows` reports `has_selection_input` (a Selection input, and the overlay's ComfyUI menu and Remix's templates also still need a picture target `run_i2i` fills), and the selection pill, the overlay's ComfyUI menu and Remix's templates leave out a workflow without one. `run_i2i` refuses it as well. A workflow whose binding sits on a loader detection does not recognise has no detected input and stays offered on its binding. **`run_i2i` and `run_t2i` do not read the modes**: the overlay's ComfyUI menu and Remix templates still fill only the one target `run_targets` names. The run route below (#1307) is the one that fills inputs by mode, and a workflow with no Selection input runs from the toolbar through it.
...
| POST   | /api/v1/libraries/{library_uuid}/discard                                      | libraries       | Give a folder back: undo a first import                     |
| POST   | /api/v1/libraries/{library_uuid}/promote                                      | libraries       | Start an empty library here, without importing              |
...
- **`TaskRunner`** — executes tasks from CPU and GPU queues. CPU queue is multi-threaded (`NUM_WORKERS` per `worker_config.py`); GPU queue is serialised on one worker thread, which avoids CUDA contention and is also what keeps Apple Metal work on a single thread (below).
- **`WorkPlanner`** — polls each finder, respects `*_MAX_INFLIGHT` limits, applies adaptive backoff when no work is found.

**On Apple Metal the GPU worker is the only thread allowed to touch the device.** torch's MPS backend crashes (`SIGSEGV`, a Metal assertion) or hangs when two threads use it at once — a first pass on cold kernels, a `torch.mps.empty_cache()` while another thread runs a model — and no `try`/`except` sees it; the evidence is in `docs/apple-metal-thread-safety.md`. The single GPU worker already serialises GPU tasks, so it is the one Metal thread. Code that is not a task reaches it with **`TaskRunner.run_on_gpu_worker(fn, *args, timeout_s=…, **kwargs)`**, which wraps the call in a `GpuCallTask` (`tasks/gpu_call_task.py`: GPU queue, `URGENT`) and blocks until it settles, so call it off the event loop:

- **Inline on the worker.** Called from the GPU worker itself (`is_gpu_worker_thread()`, the thread recorded by `start()`), `fn` runs directly: a GPU task using a helper that routes through here would otherwise queue behind itself and time out.
- **Errors arrive as themselves.** Whatever `fn` raised reaches the caller unwrapped (the task keeps it in `GpuCallTask.exception`), so a direct call moved onto the worker keeps its `except` clauses. A runner that is stopped, was never started, or whose GPU worker has died raises `TaskRunnerNotRunningError` (a `RuntimeError`) at once rather than running `fn` on the calling thread or waiting out the timeout, and a caller already waiting gets the same error within `GPU_CALL_LIVENESS_POLL_S` of the worker dying; a call cancelled while queued raises `TaskCancelledError`; one still queued at `timeout_s` (default `GPU_CALL_TIMEOUT_S`, 60 s; `None` waits as long as the worker lives) is cancelled, never runs, and raises `TimeoutError`. `submit_and_wait` keeps its older contract (failures wrapped in `RuntimeError`) for its face-detection callers.
- **What a call leaves is freed on the worker.** The caller drops the error or result on its own thread, and a Metal tensor freed there is Metal work off the worker. So before the caller wakes, a call that raises clears the finished frames its error holds (through `__cause__`, `__context__` and exception groups) and runs `gc.collect()`, and a call made with `collect_garbage_after=True` collects once it returns. `Vault.run_long_inference` passes it, since a plugin's model can sit in a reference cycle.
- **Nothing moves or parks it.** CPU spillover is a flag a task reads to choose the CPU device, never a move to another thread, and `_run` skips the VRAM gate that offers it for every GPU-queue task; a `GpuCallTask` also declines spillover and estimates 0 MB, so the gate would pass it at once anyway.
- **It gets the OOM retry.** Like any GPU-queue task a call is re-run on a device OOM, so `fn` must be safe to call again, as an inference pass, a model load or a cache flush is. An inline call has no retry of its own: an OOM it raises is the enclosing task's to handle.
- **No flush after it.** `_run` flushes the device cache and runs `gc.collect()` after a GPU-queue task only when the task's `FLUSH_DEVICE_CACHE_AFTER_RUN` is true (the `BaseTask` default). `GpuCallTask` sets it false: a flush per call costs a collection and returns the buffers the next call reuses.

**Code outside a task calls `Vault.run_inference(fn, *args, **kwargs)`, not the runner.** It decides whether routing applies: on Metal (the engine's `device` is `mps`, or no engine was built on a host where Metal is present, since `fn` then picks its own device) with a task runner, the call goes to `run_on_gpu_worker` and blocks; on CUDA, the CPU, or a Vault with no task runner (`disable_background_workers`, which has no worker thread to race) it runs inline, unchanged. The arguments are bound with `functools.partial` first, so a keyword meant for `fn` is never taken for `timeout_s`. **`Vault.run_long_inference`** routes the same way with no timeout and no GPU out-of-memory retry (`run_on_gpu_worker(..., retry_vram_oom=False)`), for a run the user started and watches progress for: a retry would rerun it and report its progress twice. A `GpuCallTask` keeps whatever its call raises for the waiter, `SystemExit` and `KeyboardInterrupt` included, and a plugin's `sys.exit()` does not end the GPU worker (`BaseTask.run` records it as the task's failure, below). The callers are the anomaly-region tagger load and Grad-CAM pass (one call, so an idle unload queued on the worker cannot land between them) and, through `run_long_inference`, every image plugin's `run` / `run_video` (`image_plugins/service._run_plugin`): the plugin API does not say whether a plugin uses the GPU, so on Metal a PIL filter waits behind GPU tasks too. `GET /pictures/{id}/anomaly_region` turns a routed `TimeoutError` into 503 ("the GPU is busy") and a `TaskCancelledError` or `TaskRunnerNotRunningError` into 503 ("the GPU worker is not running"), without retrying; a `RuntimeError` from the model keeps the route's own status. `POST /pictures/plugins/{name}` maps the same two runner errors to 503 (`plugin_service.run_plugin_on_pictures` re-raises them unwrapped) and turns a plugin's `SystemExit` / `KeyboardInterrupt` into the ordinary 500 "Plugin failed", because re-raised on the event loop asyncio would stop the server. The idle unload (`Vault._maybe_aggressive_unload`, reached from the worker-progress poll) must not block the poll, so on Metal it submits a `GpuCallTask` and returns; a stopped runner skips the unload with a warning rather than running it inline. Two task-side paths that used Metal off the worker are closed at the source: `TagTask.on_queued` does not preload its model on a side thread when the engine is on Metal, and `FaceExtractionTask.release_detection_models` (also run by the face finder's drain on the planner thread) flushes only CUDA. Off Metal the idle unload runs on the poll thread, and an unload there flushes Metal's cache only for a service on Metal, so a Mac forced onto the CPU flushes nothing from it.

**Search encodes its query on CPU copies, never on the GPU worker.** Text search (`Vault.generate_text_embedding` and `generate_clip_text_embedding`), export by query (`generate_text_embedding`, in its background task) and likeness search's image encode (`_likeness_search._encode_query_image`, run in an executor so the event loop never waits) encode on their own thread with whatever `Vault.query_encoders()` returns. On Metal that is `InferenceEngine.cpu_query_encoders`, a `CpuQueryEncoders` (`inference/cpu_query_encoders.py`) that `InferenceEngine.create` builds only for a Metal engine: a `ClipService` and an `SBertService` of the engine's own classes on the `cpu` device, so model names, weights, preprocessing, normalisation and float32 dtype match the Metal services and a query vector is comparable with the stored ones, which GPU tasks compute on Metal. On CUDA, the CPU, or a Vault with no task runner, it is the engine itself. The rules around the copies:

- **They load on the GPU worker.** `Vault._start_loading_cpu_query_encoders`, called wherever an engine is built (`ensure_ready`, `get_worker_future`), queues `CpuQueryEncoders.load` as a `GpuCallTask` (`Vault._queue_cpu_query_encoder_load`). transformers and accelerate fail when two threads import or load models at once (`ImportError: cannot import name 'AcceleratorState' from partially initialized module 'accelerate.state'` in 3 of 10 runs that loaded the copies on a search thread while the worker loaded models), and every other model load on Metal happens on the worker, so the load waits for the task in front of it rather than running beside it. A library switch builds its engine before starting its runner; the queued load runs first once it starts.
- **A search waits for the load and never performs it.** `query_encoders()` hands out the copies only once `CpuQueryEncoders.is_loaded()`; before that it queues the load if none is pending and waits on the load task, up to `Vault.CPU_QUERY_ENCODER_LOAD_WAIT_S` (60 s). A load that does not finish in time, fails, was cancelled (`TaskRunner.cancel_pending_tasks`, which a full restore calls) or cannot be queued (runner stopped) raises `CpuQueryEncodersNotReadyError`; `GET /pictures/search` and `POST /pictures/likeness-search` turn it into 503 ("Search is still loading its models"), as do the two export starts that take a query (`GET /pictures/export`, `POST /pictures/export/folder`), which wait for the load before queueing the export and encode later, in its task. An export started before the engine existed skips that wait; its task waits for the load when it encodes, and fails with a WARNING only if that load fails or outlasts the wait. The next search queues a failed or cancelled load again. A load already pending or finished is shared. With no running GPU worker (`TaskRunner.is_gpu_worker_alive()` false) the search raises at once without queueing, and a waiting search polls liveness every `TaskRunner.GPU_CALL_LIVENESS_POLL_S`; either way the error's `worker_running` is false and the 503 (`NO_GPU_WORKER_DETAIL`) asks for a restart if it persists instead of a retry. A call on the GPU worker itself loads the copies inline. `Vault._build_engine` (both engine-build sites) queues the load before assigning `_engine`, so a finder cannot queue a GPU batch ahead of it.
- **They stay resident.** Nothing unloads them: `InferenceEngine.close` and the idle unload name only the engine's own services, so the copies (about 0.65 GB) live as long as the engine, and a search never lazily reloads a model on its own thread. `Vault.stop` drops its handle on the load with the engine.

Measurements (latency, ranking parity with Metal-computed vectors, crash runs) are in `docs/apple-metal-thread-safety.md`.

**A missed path raises instead of crashing.** `TaskRunner.start()` registers its GPU worker with `utils/device_utils.register_metal_thread`. The registration is cleared (only if it is still the registered thread) by `stop()` once the worker has exited, or by the worker as it leaves its loop; a worker still running after `STOP_JOIN_TIMEOUT_S` stays registered. `ensure_metal_thread(device)` raises `RuntimeError`, naming the calling thread and pointing at `Vault.run_inference`, when `device` is `mps`, a worker is registered, and the caller is not it; it is silent on other devices and with no running runner, and a registered worker that died of an uncaught exception still refuses every other thread, saying so. While a registered worker runs on Metal, `Vault.stop()` leaves the engine and the class-level models loaded (the engine in `Vault._engines_left_loaded`, with that worker) and still closes the database, without the `gc.collect()` its close would otherwise run beside that worker; the next `Vault.start()` queues each such engine's close on its own GPU worker once the recorded worker has exited, dropping the entry only when the close has run, and `Vault.start()` waits up to `PREVIOUS_METAL_WORKER_WAIT_S` for it before refusing (`wait_for_previous_metal_worker`), wherever Metal is present and whatever the engine; a library switch's `_bring_up` waits the same way before it builds the new engine, which reads Metal's working set. A failed `Vault.start()` stops what it started and closes the vault before re-raising. It sits at the entry points non-task code reaches, not in inner loops: `SBertService.encode`, `ClipService.encode_text` / `encode_image_batch` / `encode_image_crops`, `ModelLifecycleManager.aggressive_unload` / `safe_idle_unload` (before anything is unloaded) and `PixlStashTaggerService.localize_anomaly`. A new route that runs a model on the engine's device goes through `Vault.run_inference`; the guard is the backstop, not the mechanism.
...
**A task that raises `SystemExit` fails; its worker runs on.** `BaseTask.run()` catches a `BaseException` from `_run_task()` that is not an `Exception` (`SystemExit` from a third-party tagger's `sys.exit()` inside `TagTask`, a `KeyboardInterrupt` a plugin raised), logs it at WARNING with the task id, type and the original traceback, and raises `TaskInterruptedError` (a `RuntimeError`, the original as `__cause__`) in its place. The task is `FAILED`, `_run` logs it and fires the completion callbacks as for any failure, and the worker takes its next task. `_run` catches only `Exception`, so the original would end the worker thread: one CPU worker fewer, or the GPU queue's only worker, and on Apple Metal no other thread may run GPU work, so all of it would wait for a restart. A `KeyboardInterrupt` there is never the owner stopping the process: only a runner's worker threads call `run()`, and Python delivers `SIGINT` to the main thread. A `BaseException` raised outside that handling, such as by a completion callback, still ends the worker, and a dead GPU worker stays registered as the Metal thread (above).
...
- **A device out-of-memory only.** `is_vram_oom()` takes `torch.OutOfMemoryError` by type, or a message that says "out of memory" **and** names a device (`cuda`/`gpu`/`hip`/`vram`/`mps`), walking `__cause__`/`__context__` because a plugin wraps the driver's error in its own class. The device word is what keeps `sqlite3.OperationalError: out of memory` (SQLITE_NOMEM) out. `mps` is there, as a whole word, because Metal's OOM text — `MPS backend out of memory (MPS allocated: …, max allowed: …)` — names none of the others. Two texts count on their own: `CUDA_ERROR_OUT_OF_MEMORY` and onnxruntime's `Failed to allocate memory for requested buffer`. Metal's `Invalid buffer size` does not: it refuses one allocation larger than the device's maximum buffer, the same way on every attempt, so a retry cannot succeed. It is a Metal fault for `is_device_error()`, and the services move to the CPU on it.
...
Long-running work threads the event downward as a `stop_event`: `DescriptionTask` passes its own to `DescriptionWorkflow.generate_batch()`, which checks it per picture and hands it to the plugin, so a shutdown stops between images instead of running a whole batch of JoyCaption captions out. **The event is a parameter, never workflow state:** with CPU spillover the batch runs on a `DescriptionWorkflow` that `InferenceEngine.description_workflow` builds fresh on every access, so an event stored on the workflow would be set on an object nobody is running. (`TagTask` does not yet pass one to `TaggingWorkflow.tag_images`; the same treatment is owed there.)
...
**The captioning plugin sizes its own description tasks.** `MissingDescriptionFinder` fetches with the engine's description batch size (Florence-2's, up to 32 on a GPU), then asks the plugin that will caption the task, through `TaggerPlugin.description_task_size(device)`, how many pictures to put in it: the active description plugin for the `NULL` backlog, or the plugin a sentinel group names for a user's request. It resolves that plugin the way `DescriptionWorkflow.generate_batch` dispatches: Florence-2 keeps the engine's size without the registry being consulted, and so does a plugin the workflow would replace with Florence-2 (missing, or `supports_descriptions` false), whose hook is never asked. `None` also keeps it, and any answer is capped at it, so a plugin can only shrink a task. The reason is the one GPU worker: a task holds it until it returns, so a routed call, an urgent request or the next task waits for the whole task, however many images it carries. `JoyCaptionPlugin` returns 1 on Metal, where it captions one image at a time at 20 s or more each; CUDA and CPU hosts keep the engine's size.
...
All torch models run on CUDA, Apple Metal (MPS) or CPU, chosen by `utils/device_utils.detect_device()`. Two Metal-specific routes are deliberate, not oversights. **Florence-2 loads straight onto Metal in fp16**, as it does on CUDA, and a failed Metal load records a fallback and loads fp32 on the CPU. A direct load through `transformers`' threaded loader crashes, because its threads cast weights on Metal at the same time (`docs/apple-metal-thread-safety.md`); `configure_metal_model_loading()` keeps that loader on one thread, and fp16 is the checkpoint's own dtype. Its Metal batch is capped by memory, not the CUDA constant: `Florence2Service._metal_batch_cap()` fits `metal_base_mb + metal_per_image_mb * batch` (per variant, in `FLORENCE_MODEL_VARIANTS`) into `FLORENCE_METAL_MEMORY_FRACTION` (half) of `torch.mps.recommended_max_memory()`, read once at construction. On Metal, tag tasks and tagger batches hold at most 8 images (`TaggingWorkflow._MAX_CONCURRENT_METAL`), not the GPU's 64: no VRAM budget bounds a Metal batch and it all comes out of the machine's one pool of memory. The limit is the taggers' alone; `InferenceEngine.max_concurrent_images`, which Florence-2 sizes from, is unchanged. And the **ONNX** models take their own route — **WD14 through `CoreMLExecutionProvider`**, with `CPUExecutionProvider` listed after it (onnxruntime runs the operators CoreML declines on the CPU either way; listing it keeps the node-assignment warnings out of the log). The CoreML session is created with `enable_fallback=0`, because onnxruntime's default rebuilds a failed session on the CPU and reports the error only with `print()`: a CoreML failure, at start or as an `EPFail` during a run, is logged at WARNING with the model path, the provider options and the error, WD14 then creates a CPU session explicitly (a failed batch is retried on it once), and that service stays on the CPU across reloads. Other run errors fail the batch and leave the session on CoreML. Every other accelerator session, WD14's on CUDA, ROCm or OpenVINO and InsightFace's on CUDA, keeps onnxruntime's default fallback, so WD14 and InsightFace read `get_providers()` after the session loads and after each batch, and log a WARNING when onnxruntime has put it on the CPU: WD14 once per loaded session, InsightFace once per loaded app, whichever of its five sessions moved. ROCm is the exception, and InsightFace says nothing there: torch reports CUDA as available, but the onnxruntime that ships with the ROCm build has no CUDA provider, so its CPU sessions are the arrangement rather than a fallback. CoreML compiles the model on every load (11-13 s against 0.5 s on the CPU with the real checkpoint); the trade-off against a `ModelCacheDirectory` is recorded on `_COREML_PROVIDERS`. Meanwhile **InsightFace stays on the CPU provider**: its SCRFD detector (dynamic `[1, 3, ?, ?]` input) builds on CoreML but fails at run time at the 256 px `det_size` the face pipeline uses (with the `MLProgram` model format, `Invalid shape for output feature`).

Models are lazily loaded on first `init()` call and can be unloaded after idle to free VRAM unless `keep_models_in_memory` is set.
...
| [services/saved_recipe_service.py](../pixlstash/services/saved_recipe_service.py) | **Saved recipes: the vault side, and the credit nothing stores.** A saved recipe is the user's Recipe (prompt, LoRAs, overrides), not the hub's `workflow_recipe` variant. Credit matches the stack's kept pictures on prompt and normalized LoRA names — seeds and strengths ignored, because no picture row holds a strength — as one grouped read per stack matched in Python, so a tab of twenty recipes costs one query, not twenty |
...
| [utils/device_utils.py](../pixlstash/utils/device_utils.py) | Inference device: `detect_device` (`cuda`, `mps` or `cpu`), `is_accelerator` / `is_metal`, the allocator flush `empty_device_cache`, `configure_metal_model_loading` (§3), and the Apple Metal thread guard `register_metal_thread` / `ensure_metal_thread` (§7) |
...
**A picture from A1111 or a fork of it is filed the same way** (#1312). With no
ComfyUI graph in the file, `services/a1111_recipe.py` reads the A1111 text — a
PNG's `parameters` chunk, or a JPEG's or WebP's EXIF `UserComment`, which
`ImageUtils._user_comment` decodes out of the Exif sub-IFD `getexif()` does not
walk — and builds the graph it describes as a reduction, which
`hub/workflows.record_reduction` files exactly as `record_api_graph` files a
ComfyUI one. Every field is classified by name: **structure** is a node (a LoRA
or embedding in use, hires fix, a refiner, an img2img source); **assets** are
`Model`, `VAE`, `Refiner`, `Hires checkpoint`, LoRA and embedding names (given
`.safetensors` when A1111 wrote none), any field named for a model (`Hires
upscaler`, `ADetailer model`, Forge's `Module 1`), and the checkpoint and
embedding hashes as `*_sha256` widgets; **volatile** is the seeds, `Version` and
the hash summaries, `Lora hashes` included (A1111 hashes a `.safetensors` LoRA's
tensor data, not the file, so it could never resolve); a credential-named field
...
its `Model:` replace the checkpoint. **A short hash is kept
as written and resolved when read** (`workflow_hash.digests_with_prefix`): one
shelf digest starting with it is that model (verified), none or several leave the
picture to the filename tier. Only 10, 12 and 64 digits are read as a digest, so a
partial value in a ComfyUI widget still names nothing. The same resolution serves
`fetch_companions`, where a short hash naming several models is as ambiguous as a
shared filename. Resolving at scan time would give two pictures of
one model different recipes depending on whether the model was on the shelf yet.
A1111's 8-digit hash from 2022 is not a sha256 prefix and is dropped. Migration
`0119` hands back every PNG, JPEG and WebP stamped scanned with no keys, so
existing A1111 pictures are read once.
...
`model.sha256`, or an A1111 short hash only one `model.sha256` starts with, is
**verified**, a filename equal to `model.filename` or a copy's
...
pictures imported before #1311 without waiting for the backfill. **A
`checkpoint_id` counts in neither tier**: since #1416 it is an asset and keys
the recipe, but the value is this shelf's row id rather than a digest or a
filename, so neither resolver can match it and nothing false is claimed. Two
consequences worth stating rather than discovering: a recipe filed on one
machine names a different model on another's shelf, and `checkpoint_hash_task`
deletes the losing row when two paths hash alike, so the id of a merged model
changes under an already-filed recipe. Both argue for resolving the id to a
digest at file time; that is a design decision of its own, not a third branch
here, and until it is taken the id is the only identity the executed graph
carries.
...
vault how many of *its* pictures came from each, and merges the two answers in
Python. That is the arrangement content addressing was chosen for: a hash the
attached hub has never heard of is simply a workflow this machine does not have,
and a library detached and reattached elsewhere still lists correctly against a
hub that holds its recipes.

The list is **one row per topology**, not per recipe — ~192 rows against ~617 on
the owner's library — and the recipes under one topology are fetched separately,
when a row is opened. **Four** queries serve the whole list, whatever its length:
`topology_index` and `assets_by_topology` on the hub, `adapter_slots_by_topology`
beside them, and one vault read that answers both the per-topology counts and the
scan progress from a single session. The expand is two more (`recipes_for_topology`
and `assets_for_topology_recipes`) plus one vault read — not one per recipe, which
on the 159-variant family would be 161 statements for one disclosure.

**`assets` on a topology row is a SET, and `adapter_slots` is what a run loads.**
The asset table is keyed per recipe, so a family of 159 character LoRAs names 159
files and 159 copies of its checkpoint; the row de-duplicates that, and carries
the slot count separately because the set cannot answer it. A caller describing
the row from the set alone says the graph loads 159 adapters at once. The slot
count is a `MAX` over the topology's recipes and is **exact rather than an
estimate**: a topology is the graph alone, so every recipe filed under one has
the same `lora_name` inputs by construction, and the `MAX` is only there so a
recipe whose names were forgotten cannot drag the answer to zero.

**Every route in the module is `OWNER_ONLY`, and that is a decision.**
`topology_activity` groups every non-deleted picture in the vault, so a picture-,
...
exists for — and `/workflows/{topology_hash}/pictures` returns vault picture ids
...
#### Saved recipes (v1.12, B6)

A **saved recipe** is the look somebody pressed Save on — prompt, LoRAs with
strengths, overrides — and is not the hub's `workflow_recipe` row, which new code
calls a *variant*. It is a **vault** row (`db_models/saved_recipe.py`, migration
`0121_add_saved_recipe`) because it is authored rather than derived: the hub's
...
the next import; a CHECK holds that a `fixed` row has one, and any other row may
keep the picture it had. Forgetting a library deletes its rows with its ghosts.
...
**Every other hard delete reaches the cascade through the vault, not through
its caller (#1293).** Picture rows are also removed by `MissingFilePurgeTask`,
startup missing-file cleanup (`maintenance.py`), reference-folder removal and
...
existed), and a full restore re-queues every ghost of the library, because the
file swap drops pictures without deleting a row. A hubless vault keeps its queue
until it is opened through a hub.

**Bounded on purpose.** A drain re-evaluates the instance hashes that were given
...
well as its name does). **A `checkpoint_id` is never a ghost and can never be
forgotten** (#1416): it names nobody, so there is nothing to purge, but the
corollary is that a shelf row it points at which has since been merged away or
forgotten leaves an asset value no purge reaches. Resolving ids is the same
open decision §5's picture counts name. A digest is judged only when it is 64 hex characters and
...
2. `StartupChecks().run()` validates disk space, VRAM, GPU (CUDA / Metal), SSL; may force CPU mode.
3. `Server.__init__()`:
...
- **The library's own root is scanned by the same task, under `folder_id=None`.**
  ...
  and there is no status. **Caption-file sync for the root** comes from the
  same `LibrarySettings` row (`sync_tags`, `sync_descriptions`, `tags_suffix`,
  `description_suffix`, migration 0116): with a kind on, the root gets the
  reference folder's reconcile pass for that kind only (a sidecar edited on
  disk is read in, a picture with content and no file gets one exported under
  the stored or default suffix), an unset suffix is detected over the walked
  tree and persisted, and `caption_utils.sync_picture_sidecar` writes a
  managed picture's edits back by the same settings. A picture's recorded
  file wins while it exists (`recorded_sidecar`); a file on disk the picture
  never recorded is left for the scan (`writeback_target`), never replaced by
  an edit; a pending-description sentinel never reaches a file
  (`description_caption_content`); and one suffix for both kinds is refused
  everywhere a suffix is stored (`suffixes_collide`). **The root scan
...
**Keep recipes only (#1315) is a mode of the same planner, not a second one.**
`keep_recipes` on either route passes a `RecipeCheck` (hub, library, ghost
...
`utils.library_roots.refuse_overlapping_folder`, so a root that equals, sits
inside or contains another registered library, a watch folder or another
reference folder is refused too - one step ahead of the conflict check, and
naming the folder in the way. This path passes `exclude=root_path`, since
registering is idempotent by path and the row a resumed commit made last time
...
**Confirming a pattern on import turns that kind's root sync on.**
`local_import_pictures` ends by calling `seed_caption_suffixes` with the first
confirmed suffix per kind: the toggle goes on, a suffix already stored wins,
a kind not confirmed is untouched, and a suffix that would name the other
kind's file is refused and logged. When that write left a kind on that was
off, `Vault.rescan_library_root` queues the pass that reads the owner's files
in before any write-back could replace one. Settings turns it off again.
