...
The authz refactor (§16.2) moves this class off `require_user_id` and onto declared `AccessPolicy` tiers in `pixlstash/authz/registry.py`. The 16 host-capability routes split into two tiers. (Declared and armed behind the report-only gate; runtime behaviour is unchanged until the Step-6 `AUTHZ_GATE_ENFORCING` flip.)
