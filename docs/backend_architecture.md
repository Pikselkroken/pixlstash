...
`workflow_topology`, `workflow_recipe` and `workflow_document` are hub tables,
...
`workflow_document` holds the **structural** document, not the file as it
arrived: parameters and seeds are already nulled and any field named like a
credential is dropped. That is what makes the plan's §5 deletion boundary real
rather than aspirational — "forget the pictures" purges instances and ghosts and
leaves the recipe standing, with no purge having to reach in and rewrite a
stored graph. The verbatim import store is a different thing on a later step and
belongs beside the workflow file. Its unique index is
`(structural_hash, format)`, so exactly one document survives per recipe per
format: the same workflow rebuilt from scratch has different node ids and so
different document *text* at the same identity, and the content-addressed
primary key alone would let those accumulate.
...
Measured over the owner's three libraries (28,289 images carrying an API graph):
625 structural recipes over 202 topologies, 71 recipes covering 90% of the
images, and the top 25 topologies covering 79.6%. All six of the hash spec's
§Node identity invariants hold on 400 real graphs drawn at random. 21.4% of
those API graphs carry colon-path ids and 20.0% of the UI chunks define
subgraphs, matching the spec's own figures.
