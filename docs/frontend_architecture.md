...
- **The chip is the design's `.uchip`**, not the DS Tag: a control-tier chip with a 1px `border` outline on the `input-background` surface, `--space-2`, `--text-2xs`, the label at full ink, and a 12px muted glyph naming what kind of thing it is (`cube-outline` a checkpoint, `layers` a workflow LoRA, `plus` a slot). F0 first shipped these as `Tag` xs — a 10% wash, muted, unbordered and glyphless — which is the DS component for an image *tag*; a model chip is a different object and the design draws it differently. Three variants, all from the design: a **recipe slot** is dashed and unfilled, a **fact** keeps the outline and drops the fill, and **"+N"** is a fact chip with the count muted. Height stays `--tag-h-xs` (18px) where the design draws 20, rather than minting a fourth chip height for 2px; nothing else about the chip is approximated. ⓘ's Differs-by chips are the same fact chip, so what the card clipped and what the popover lists read as one list.
