...
- **Filters** (F7) is a `.tbm` panel on the toolbar's funnel,
  ...
  ways to get a first workflow and this reader already has some.
- ***Show all N pictures*** (F7) is the picture figure in `WorkflowTab`'s
  ...
  strip. `workflow_key` and not `workflow_stack` even on a stack's cover,
...
(`WorkflowRunPanel` is the `v-if`, `WorkflowTab` the `v-else-if`): starting one
...
`useWorkflowRunStore().open`, as it mounts `WorkflowTab` on `/workflows`.
