...
- `OverlayRecipePanel.vue` (~430 lines) — the **Recipe** section (#1313), read-only, between Tags and Metadata. Fed entirely from the `comfyMetadata` ImageOverlay already fetches from `GET /comfyui/pictures/{id}/workflow`, so the section costs no second request. Shows the models as chips carrying the strength each LoRA was loaded at and a `mdi-check-decagram` badge when the recipe named the file by its digest (a chip with a `model_id` is a button to `/models?model=<id>`; one without is the same chip inert, because "not on your shelf" is information); the prompt; the sampler settings; the **resolution lock** — thumbnails of the pictures the run actually loaded, dashed and iconised once one has left the library; **Generate variants…**, which emits `open-remix-dialog` up to ImageGrid's existing `RemixDialog` rather than hosting a second one; and the workflow JSON with Copy, Download and **Open** (→ `/workflows?topology=<hash>`). The section is absent entirely for a picture whose file carries no ComfyUI workflow, which is the normal state of an imported photo.
