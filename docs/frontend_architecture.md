...
Shared application chrome. In a plain browser it renders a compact owner-only active-library control that deep-links to Settings › Libraries; in Electron that control is part of the custom title bar alongside the `WordmarkLogo`, breadcrumb trail (`useBreadcrumb`), app version, update alert, and OS window controls. All desktop calls go through `window.pixlstashDesktop?.…` optional chaining. Share/read-only sessions receive neither the library name nor its entry point. Props: `installType`, `checkForUpdates`, `activeLibraryName`, `showLibraryChrome`; emits `open-libraries`.
