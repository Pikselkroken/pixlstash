...
is a tab stop even so: the pencil stays at `tabindex="-1"` and F2 is the
keyboard path, because the alternative is 1,800 new stops.
...
high — the column headers, a group header, the band legend — paints the same
text just as well. It is the app-chrome rule `style.css` already applies to the
