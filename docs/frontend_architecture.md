...
2. **Deferred pills + deferred grid mutation.** Nothing reshuffles the grid or raises a pill under an open overlay:
   ...
   - The deferral covers the **grid**, not the overlay's own content. A change to something the lightbox itself renders must therefore reach it through a dedicated signal, or it stays stale until the overlay is closed and reopened: `wsSmartScoreUpdate` (metadata panel score), `wsDetectionUpdate` (object boxes, re-read from `/pictures/{id}/detections` — otherwise a Segment run started from the overlay context menu showed nothing until reopen), `wsTextUpdate` (the text read from the picture) and `wsPixelsUpdate` (the picture's own bytes — see below) exist for that reason. The score and detection signals fire on any distinct signal key while a card is open, without gating on the payload's `picture_ids`, because signals written in one Vue flush coalesce to the last one, which may not name the open card; the text and pixel signals ARE id-gated, because each carries one gesture on named pictures.

   - **`wsPixelsUpdate` — the picture's bytes (#1419).** A rotate made in the lightbox refreshes the lightbox itself (`ImageOverlay.refreshAfterPixelChange`). The **undo or redo** of that rotate has no such local hook — it arrives only as a `pictures_changed` frame with `fields: ["pixels"]` — and neither does a rotate made in another tab, so the picture stayed the wrong way up until the lightbox was closed and reopened. `useUpdatesSocket` raises `wsPixelsUpdate` off that field and the overlay runs the SAME `refreshAfterPixelChange`, so a turn from any origin costs exactly the same reads. **Five producers stamp `pixels` and only two are turns** (`integration_architecture.md` §8.3 has the table), so that function always re-reads metadata — for `orientation`, which is what `fullImageSrc`'s `?v=o<n>` is built from — and re-reads `/faces`, `/detections` and `/text` only when that read shows the orientation actually MOVED. A background thumbnail batch must not clear the viewer's word selection, which `pictureText.refresh()` does. Like the score and detection signals and for the same coalescing reason, it is **not** gated on the payload's `picture_ids`: `pixels` is the most batched of the four (200 ids from a rotate, 64 from the thumbnail sweep), so the frame naming the open card is the likeliest to be the one a flush drops.
