...
**The search half of the grid action pill** — a run of controls, not a surface. Props: `imagesLoading`, `statusCount`, `statusLabel`, `isAllPicturesActive`, `ownsEscape`, plus the person-search set below. Emits: `clear`, `search-all`, `update:threshold`, `assign`.
...
- A **threshold** range input (`threshold`, `thresholdMin`, `thresholdMax`), same native-input pattern as `DedupTierMenu` — real `<label for>` plus an `<output>` in percent, so it is keyboard-operable and named. It emits on `input`, not `change`: the count has to track the drag, not wait for the pointer release. `thresholdMin` is the **fetch floor**, since below it there are no fetched results to reveal. **The label IS the readout** (`Match ≥ 82%`), one run rather than label + gap + track + gap + output. It has **two forms**, swapped by the container query and never both operable (`display: none` keeps the inactive one out of the tab order): inline wherever there is room for real travel, and a value-carrying trigger + popover below 780px and on **every coarse pointer**. **Vertical was rejected on arithmetic** — 46 discrete steps in a 40px band is ~0.9px per step, a 4-8x precision regression, and the target size WCAG 2.5.8 wants cannot coexist with the travel.
