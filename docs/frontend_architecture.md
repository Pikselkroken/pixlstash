...
- `OverlayMetadataPanel.vue` (267 lines) — metadata, score, dates, file info, penalised-tag indicator. **One list, no tab band:** the ComfyUI tab that used to sit beside Picture information moved whole into `OverlayRecipePanel` (#1313), which is where everything about how the picture was made now lives.
- `OverlayRecipePanel.vue` (~555 lines) — the **Recipe** section (#1313), read-only, between Tags and Metadata. Fed entirely from the `comfyMetadata` ImageOverlay already fetches from `GET /comfyui/pictures/{id}/workflow`, so the section costs no second request. Shows the models as chips carrying the strength each LoRA was loaded at and a `mdi-check-decagram` badge when the recipe named the file by its digest (a chip with a `model_id` is a button to `/models?model=<id>`; one without is the same chip inert, because "not on your shelf" is information. `ModelShelf` honours that query once per value and, when the shelf holds the row but is not drawing it, says so in a notice offering *Show everything* rather than landing at the top of the list having visibly done nothing — it does not clear the reader's filters behind their back, which is where it deliberately differs from `WorkflowShelf`, whose one Show axis it is safe to widen); the prompt; the sampler settings; the **resolution lock** — thumbnails of the pictures the run actually loaded, dashed and iconised once one can no longer be shown (the server nulls `input_picture_id` for a binned input, and an `@error` on the `<img>` catches a thumbnail that simply will not load, so the browser's broken-image glyph never stands in for the designed tile). **The lock is owner-only and arrives empty for a share link**, because a row names another picture's id and content hash; **Generate variants…**, which emits `open-remix-dialog` up to ImageGrid's existing `RemixDialog` rather than hosting a second one; and the workflow JSON with Copy, Download and **Open** (→ `/workflows?topology=<hash>`). The section is absent entirely for a picture whose file carries no ComfyUI workflow, which is the normal state of an imported photo.
