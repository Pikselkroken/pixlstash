...
1. **The migration is one new decision, not a new engine.**
   ...
   being true — and it keeps `relocate`'s two properties so the tree is not read
   two different ways: the owner's own subfolders travel, and the unfiled folder
   is not a level to nest under, at any depth. **It also keeps
   `match_destination`'s override.** A folder the layout owns no part of is a
   permanent override, and it has to stay one here or the one gesture that runs
   over the whole library would be the one that undoes every override in it. The
   flat library is not that case: a picture at the root is in no folder, so it
   overrides nothing, and it is what this phase exists for.
