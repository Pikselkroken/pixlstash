...
### Phase 7 — PixlStash Views *(WITHDRAWN 2026-09-07)*

> **Removed, do not rebuild from this section.** The backend and the two
> `/server-config/views` routes shipped in v1.11, but the settings section was
> never wired into the app: `ViewsSection.vue` was imported by nothing except
> its own test, so no user could ever reach the feature. Rather than finish an
> integration nobody had asked for in a release cycle, the whole thing was
> deleted. Two things outlived it: `docs/spikes/views-links.md`, whose
> filesystem measurements bind any future link-based feature, and the
> `.pixlstash-views` prune in `ReferenceFolderScanTask`, which keeps a tree
> published through the live v1.11 API from being indexed twice.
