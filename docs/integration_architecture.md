...
| `vram_oom` | A GPU task ran out of VRAM: emitted before each retry, then once more to close the sequence | `attempt`, `max_attempts`, `gave_up`, `recovered`, `task_type` (diagnostic only) | One keyed notice (`vram-oom`), updated in place by the later frames, saying how many attempts have been used. Exactly one closing frame: `recovered` (a later attempt succeeded) or `gave_up` (the task failed and its work is left for a later sweep). The retry frames carry an explicit timeout longer than the backend's pause, or the card would expire between frames and stop coalescing. |
