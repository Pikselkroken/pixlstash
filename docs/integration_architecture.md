...
(#1310) is every LoRA a run can swap, `by` being `filename` for a core loader
and `digest` for a ComfyUI-PixlStash one; empty means the graph has no LoRA
loader, and the `PUT` answers with the same field.
...
LoRAs would otherwise load the chosen one twice: more than one slot with no
`lora_node_id`, and a `lora_node_id` that is not one of them, are both 400. No
