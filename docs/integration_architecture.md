...
3. **A model name the shelf does not hold does not travel, and that is
   stricter than the *Keeps something deleted* filter on purpose.** Forgetting a model's name
...
4. **An empty `assets` list is a state, not a missing field.** Forgetting a
   ...
   shelf) are what the Filters menu's *Keeps something deleted* row filters on, client-side.
