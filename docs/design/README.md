...
| `design-tokens.css` | The machine-readable tokens: spacing, radius, type ramp, elevation, motion. A symlink to the shipped `frontend/src/styles/design-tokens.css`, so spec and build cannot drift. |
| `design-system-handoff.md` | What is still open: design decisions needing a UI/UX call, and findings measured and decided but not yet implemented. |
