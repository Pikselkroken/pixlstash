 ...
 *   A `null` suffix means none confirmed or detected yet; the default then
 *   names new files.
 */
 ...
 * `undefined` keeps its stored value; an empty suffix clears it. Turning a
 * type on makes the server rescan the library root. Rejects with a 400 when
 * a suffix is not a bare filename fragment or both types would share one.
 *
