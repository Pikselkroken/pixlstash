- Caption files beside the pictures in your own library folder can now be
  kept in sync, the way reference folders already could. Settings, the open
  library's menu, "Caption files…": turn on tags, descriptions or both, and
  edits you make in PixlStash are written to the file beside the picture,
  while a file you edit on disk is read back in on the next scan. The naming
  you confirmed when the folder was imported is used, so nothing is written
  under a second name. Confirming a pattern on import turns its sync on; the
  dialog turns it off.
- Tags and descriptions can no longer be given the same file name suffix,
  which would have made one file hold both.
