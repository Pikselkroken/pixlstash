- Clone a workflow onto a different model: select a workflow, choose "Clone
  ...
  suggestion came from. Pulling from ComfyUI also reads its recent run
  history, so a checkpoint you have only used in ComfyUI gets suggestions too.
  Change any of them, name the copy, and it lands as a
