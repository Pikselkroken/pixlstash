...
class StartupChecks:
    ...
    def _handle_gpu_check_failure(
        ...
        if is_explicit_gpu and not is_auto_mode:
            ...
            # carries the config path and the values that start the server -
            # the one that uses this host's Metal, or cpu and auto elsewhere.
            outcome.hard_failures.append(
    ...
    def _metal_onnx_note(self) -> str:
        ...
        if "CoreMLExecutionProvider" in self._onnx_providers(ort):
            return (
                ...
                "its detector fails on CoreML at the input size PixlStash uses."
            )
        return (
