...
def relocatable_identity(folder) -> Optional[tuple[str, str, str]]:
    ...
    Two do. The managed store, which is storage PixlStash owns outright, and the
    folder PixlStash downloads its own engines into, whose location became a
    recorded setting in #905 (closing the long-open #112). The other two declared
    roots do not: the InsightFace packs have a setting PixlStash does not write
    yet (#906), and the HuggingFace cache is ``fixed`` — its location is read
    from the environment at import by a library shared with every other tool on
    the machine, so "moving" it is a restart and a re-download.
    ...
    if is_builtin_model_dir(folder["path"]):
