"""Service for picture statistics aggregation (/pictures/stats).
    ...
    def compute(session: Session) -> dict:
        ...
        if params.set_filter_ids:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="Invalid character_id"
                ) from exc
            pic_q = pic_q.where(
                exists(
        ...
        if params.import_source_folder:
            pic_q = pic_q.where(
                Picture.import_source_folder == params.import_source_folder
            )
        ...
        if params.penalised_tag_set:
            top_tags_q = top_tags_q.where(
                func.lower(Tag.tag).in_(params.penalised_tag_set)
            )
