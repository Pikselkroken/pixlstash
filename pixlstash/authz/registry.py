...
ROUTE_POLICIES: dict[tuple[str, str], RoutePolicy] = {
    ...
    ("PATCH", "/api/v1/server-config/captions"): RoutePolicy(
        ...
        justification=(
            ...
            "asks for the root scan that does the first export. The suffix is "
            "appended to picture paths on every later write, so it is validated "
            "to a bare filename fragment here, as PATCH "
            "/reference-folders/{folder_id}'s is; that route is local_owner_only "
            "for these same fields. Owner + loopback/LAN/Tailscale, or remote "
            "owner iff allow_remote_host_ops=true (§16.3.1)."
        ),
    ),
