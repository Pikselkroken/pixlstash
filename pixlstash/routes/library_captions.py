...
folder indexed in place, a toggle and a filename suffix per sidecar kind, for
the pictures in the library's own root. Turning a kind on asks for a root
rescan, the pass that reads existing sidecars in and writes the missing ones
out; from then on an edit in PixlStash is written back through
``caption_utils.sync_picture_sidecar`` as it is for a reference picture.
"""
...
def _suffix(value: Optional[str]) -> Optional[str]:
    """Blank clears the suffix; anything but a bare filename fragment is a 400,
    the rule ``PATCH /reference-folders/{folder_id}`` enforces for the same
    reason: the suffix is appended to a picture path to find the file."""
...
def create_router(server) -> APIRouter:
    ...
    @router.get(
        ...
        description=(
            ...
            "filename suffix each kind uses. Both are off until the owner turns "
    ...
    @router.patch(
        ...
        description=(
            ...
            "value. Turning a kind on asks for a root rescan, which reads every "
            "existing sidecar of that kind in and writes one beside every "
            "picture that has content but no file yet; an empty file is never "
            "created. Turning a kind off leaves every file where it is.\n\n"
            "A suffix must be a bare filename fragment (letters, digits, `.`, "
            "`_`, `-`); anything else is 400. An empty or `null` suffix clears "
            "it, so the next scan detects the convention on disk again or the "
            "default is used. The two kinds may not end up with the same "
            "suffix, their own or the default: one file cannot hold both."
        ),
        response_model=CaptionSyncResponse,
        responses={
            400: {
                "description": (
                    "A suffix is not a bare filename fragment, or both kinds "
    ...
    def patch_caption_sync(request: Request, body: CaptionSyncPatch = Body(...)):
        ...
        if not fields:
            return _response(get_caption_sync(server.vault.db))
        try:
            stored, due = set_caption_sync(server.vault.db, **fields)
