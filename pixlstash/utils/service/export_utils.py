...
class ExportUtils:
    ...
    def generate_zip(server, request, task_id, export_tasks, background_data):
        ...
        try:
            ...
            # encoders are not ready, but lets one through before the engine
            # exists. By the time this runs the engine can be on Metal, where
            # the encode waits for the encoders' load and raises only if that
            # load fails or outlasts the wait. A warning: the export itself did
            # nothing wrong.
            export_tasks[task_id]["status"] = "failed"
    ...
    def generate_folder_export(server, request, task_id, export_tasks, background_data):
        ...
        try:
            ...
            # See generate_zip: the start route cannot check before the engine exists.
            export_tasks[task_id]["status"] = "failed"
            logger.warning(
                "Export task %s cannot encode its search query yet (query=%r): %s",
                task_id,
