...
// into a directory hicolor does not declare (our 1024x1024 one, #1073) is
// invisible to every icon-theme lookup, so the desktop entry renders blank.
const HICOLOR_SIZES = [
  16, 22, 24, 28, 32, 36, 48, 64, 72, 96, 128, 192, 256, 384, 512,
];
...
describe("linux icon set", () => {
  ...
  it("offers only square icons at sizes hicolor declares", () => {
    for (const name of readdirSync(iconsDir)) {
      ...
      assert.ok(
        HICOLOR_SIZES.includes(size),
        `${name} is not a hicolor-declared size`,
      );
      assert.deepEqual(pngSize(join(iconsDir, name)), {
        width: size,
        height: size,
      });
    }
