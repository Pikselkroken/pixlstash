...
**When the coverage floor fails on your own branch, the workflow dispatch cannot fix it.** It only harvests an *already-green* `backend` gate run, and if your PR's own gated file(s) are what tipped the map under 90%, your PR's own run is the one that is red — there is no green run of it yet to harvest. `scripts/record_test_durations.py`'s own docstring documents the other input path: run the missing files locally with the gate's own flags (`--force-cpu --fast-captions --durations=0 --durations-min=0`, matching `PYTEST_FLAGS`) and feed the output through the script. Two things to get right: the script **rebuilds the whole document from only the input you give it**, so pointing it at just the newly-missing files' output and letting it write `tests/ci_test_durations.json` directly would silently drop every other test's recorded time — read the existing map first and merge the new entries into it (`durations.update(...)`) before writing, rather than trusting the script's own `--output` to do that for you. And `--force-cpu` is otherwise the CI-only flag this file tells you not to pass locally (see below) — here it is not optional: the whole point of the measurement is to model the CPU-only runners' timing, not your own GPU's. `recorded_from` in the JSON should say the run was local rather than implying it came from a CI harvest, the same way `tests/test_library_insights.py`'s entry already does.
