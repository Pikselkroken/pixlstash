...
def test_0121_creates_saved_recipe_on_a_fresh_and_on_an_existing_vault():
    ...
    same run covers both: the table is dropped to make the second case, and the
    picture the foreign key points at is left in place so a SET NULL has
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with contextlib.closing(sqlite3.connect(db_path)) as conn:
            # Fresh: the baseline already built it, and the migration ran over
            # a database that had it.
            assert _has_table(conn, "saved_recipe")
        ...
        with contextlib.closing(sqlite3.connect(db_path)) as conn:
            ...
            indexes = {
                row[1] for row in conn.execute("PRAGMA index_list(saved_recipe)")
            }
            assert {
