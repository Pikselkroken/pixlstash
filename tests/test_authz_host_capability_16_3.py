...
def test_host_capability_tier_split_is_33_local_6_loopback():
    """The loopback tier is the 4 file-manager spawns, the process restart and
    the e2e test hook; the filesystem/folder routes stay LOCAL_OWNER_ONLY. 39
    routes carry a locality tier = 33 local + 6 loopback.
    ...
    39 = 33 + 6 with the two ``/server-config/views`` routes, PixlStash Views
    ...
    assert len(local) == 33, sorted(local)
